...
_URL_GATE_MSG = {'no enabled urls': ('⚠ No enabled URLs — check a URL row to use it for jobs', 'warn'), 'no checked url linked to a tab': ('❌ No checked URL is linked to a live tab — press Reparse or Connect on a checked row first', 'error')}
...
class Bridge(QObject):
    ...
    def __init__(self, config_manager: ConfigManager, state_path: Path, cdp_client=None, parent=None):
        ...
        try:
            ...
            cfg = WatcherConfig(enabled=bool(self.config.get_state('watcher_enabled', False)), check_interval_ms=int(self.config.get_state('watcher_interval_ms', 2000)), captcha_timeout_sec=int(self.config.get_state('watcher_captcha_timeout_sec', 300)), generation_timeout_sec=int(self.config.get_state('watcher_generation_timeout_sec', 600)), auto_pause_jobs=bool(self.config.get_state('watcher_auto_pause', True)))
            self._watcher = WatcherService(config=cfg, cdp_controller_getter=lambda: self._get_watcher_cdp_controller(), job_runner_getter=lambda: self, logger=lambda msg, level='info': self._log(f'[Watcher] {msg}', level))
    ...
    def _is_pool_page_ready(self, tab_id: str) -> bool:
        ...
        return page is not None and getattr(page, 'url', '') and getattr(page, 'title', '')

    async def _build_pool_page_info(self, tab_id: str):
        ...
        return PageInfo(tab_id=tab_id, ws_url=ws, title=title, url=url)

    async def _ensure_pool_page(self, tab_id: str):
        """Register primary tab for cooldown tracking (batch self-sufficiency)."""
        try:
            if self._is_pool_page_ready(tab_id):
                return
            info = await self._build_pool_page_info(tab_id)
            from app.services.cooldown_service import ensure_pool_page
            ensure_pool_page(self._page_pool, info)
    ...
    def _log_restore_miss(self, tab_id: str, page_url: str, entries: dict, report: dict):
        ...
        if report.get('live', 0) == 0:
            ...
            if not report.get('exists'):
                ...
                self._log(f"⏳ Saved timer(s) already expired while app was closed ({len(report['dropped'])} dropped) — tab starts ready", 'info')
            else:
                self._log('⏳ Saved timers file is empty — nothing to resume', 'info')
            return
        want = f'{tab_id[:12]} / {page_url[:60]}'
        have = ', '.join((f"{k[:8]}:{(v.get('url', '') if isinstance(v, dict) else '')[:40]}" for k, v in list(entries.items())[:5]))
        self._log(f"⚠ Cooldown restore missed for {want} — {report['live']} live saved timer(s) for other tabs ({have}); tab ids/URLs changed since save?", 'warn')
    ...
    def _emit_job_action_status(self, ctx: dict):
        try:
    ...
    def _arena_to_js(self):
        ...
        urls_js = []
        for u in d.get('urls', []):
            urls_js.append({'id': u.get('id'), 'url': u.get('url'), 'enabled': u.get('enabled', True), 'status': u.get('last_status', 'unchecked'), 'last_error': u.get('error', ''), 'last_checked': u.get('last_checked'), 'tab_id': u.get('tab_id', '')})
        images_js = []
        for img in d.get('images', []):
            images_js.append({'id': img.get('id'), 'relative_path': img.get('relative_path'), 'absolute_path': img.get('absolute_path'), 'filename': img.get('filename'), 'status': img.get('status', 'pending'), 'selected': img.get('selected', False), 'assigned_url': img.get('assigned_url_id') or '', 'attempts': img.get('attempt_count', 0), 'output_path': img.get('output_path') or '', 'error': img.get('error') or '', 'size': img.get('size', 0)})
        prompt_js = {'template': d.get('prompt', {}).get('user_prompt', '')}
        ...
        settings_js = {'timeout_seconds': timeouts.get('page_load', 30), 'generation_timeout': timeouts.get('generation', 180), 'max_retries': settings_dict.get('retries', {}).get('max_attempts', 3), 'naming_suffix': output.get('suffix', '_AI'), 'supported_types': d.get('folder', {}).get('supported_types', ['.png', '.jpg']), 'overwrite': output.get('overwrite', False), 'highlight_duration': highlight.get('duration_seconds', 3), 'max_concurrent': settings_dict.get('concurrency', 1), 'browser': browser, 'output': output, 'highlight': highlight, 'timeouts': timeouts}
        folder_js = d.get('folder', {})
        progress_js = d.get('progress', {})
        return {'version': d.get('version'), 'urls': urls_js, 'images': images_js, 'folder': folder_js, 'prompt': prompt_js, 'settings': settings_js, 'progress': progress_js, 'run_state': d.get('run_state'), 'jobs': d.get('jobs', [])}
    ...
    def get_app_state(self):
        ...
        payload = {'theme': theme, 'state': {'grid_layout': grid_layout, 'window_states': window_states, 'undo_history': hist, 'undo_history_index': idx}}
    ...
    async def _finish_primary_tab(self, ctrl, primary_tab_id) -> None:
        ...
        try:
            ...
            if not (self._page_pool and primary_tab_id):
                return
            ctx = FinishCtx(pool=self._page_pool, bridge=self, tab_id=primary_tab_id, ctrl=ctrl, client=self.cdp)
    ...
    def set_watcher_config(self, cfg_json: str):
        try:
            ...
            interval = max(500, min(interval, 30000))
            ...
            self.config.set_state(watcher_enabled=enabled, watcher_interval_ms=interval, watcher_captcha_timeout_sec=captcha_to, watcher_generation_timeout_sec=gen_to, watcher_auto_pause=auto_pause)
            if self._watcher:
                self._watcher.update_config(enabled=enabled, check_interval_ms=interval, captcha_timeout_sec=captcha_to, generation_timeout_sec=gen_to, auto_pause_jobs=auto_pause)
            elif enabled:
                from app.services.watcher import WatcherService, WatcherConfig
                cfg_obj = WatcherConfig(enabled=enabled, check_interval_ms=interval, captcha_timeout_sec=captcha_to, generation_timeout_sec=gen_to, auto_pause_jobs=auto_pause)
                self._watcher = WatcherService(config=cfg_obj, cdp_controller_getter=lambda: self._get_watcher_cdp_controller(), job_runner_getter=lambda: self, logger=lambda msg, level='info': self._log(f'[Watcher] {msg}', level))
                self._watcher.add_callback(lambda p: self._on_watcher_state(p))
                self._watcher.start()
            self._log(f'Watcher config saved: enabled={enabled} interval={interval}ms captcha_to={captcha_to}s gen_to={gen_to}s', 'success')
            return json.dumps({'ok': True, 'config': {'enabled': enabled, 'check_interval_ms': interval, 'captcha_timeout_sec': captcha_to, 'generation_timeout_sec': gen_to, 'auto_pause_jobs': auto_pause}}, ensure_ascii=False)
        except Exception as e:
            return json.dumps({'ok': False, 'error': str(e)})
    ...
    def start_watcher(self):
        try:
            if not self._watcher:
                from app.services.watcher import WatcherService, WatcherConfig
                cfg = WatcherConfig(enabled=True, check_interval_ms=int(self.config.get_state('watcher_interval_ms', 2000)), captcha_timeout_sec=int(self.config.get_state('watcher_captcha_timeout_sec', 300)), generation_timeout_sec=int(self.config.get_state('watcher_generation_timeout_sec', 600)), auto_pause_jobs=bool(self.config.get_state('watcher_auto_pause', True)))
                self._watcher = WatcherService(config=cfg, cdp_controller_getter=lambda: self._get_watcher_cdp_controller(), job_runner_getter=lambda: self, logger=lambda msg, level='info': self._log(f'[Watcher] {msg}', level))
                self._watcher.add_callback(lambda p: self._on_watcher_state(p))
    ...
    def set_cooldown_config(self, cfg_json: str):
        try:
            ...
            self.config.set_state(cooldown_enabled=enabled, cooldown_min_seconds=min_s, cooldown_captcha_penalty_seconds=pen_s, cooldown_rate_limit_penalty_seconds=rl_s)
            self._log(f'Cooldown set: enabled={enabled} min={min_s // 60}m penalty={pen_s // 60}m per captcha limit={rl_s // 60}m', 'success')
            return json.dumps({'ok': True, 'config': {'enabled': enabled, 'min_seconds': min_s, 'captcha_penalty_seconds': pen_s, 'rate_limit_penalty_seconds': rl_s}}, ensure_ascii=False)
        except Exception as e:
            return json.dumps({'ok': False, 'error': str(e)})
    ...
    async def _settle_captcha_at(self, ctrl, tab_id, correlation_id, source):
        ...
        outcome = await handle_captcha(CaptchaCtx(ctrl=ctrl, pool=self._page_pool, bridge=self, tab_id=tab_id, source=source, stop=stop, log=log))
    ...
    async def _do_connect_page_pool(self, ws_url: str):
        try:
            ...
            from app.browser.page_status import PageInfo
            import re
            m = re.search('/devtools/page/([^/]+)$', ws_url)
            ...
            for img in selected_images:
                ...
                for block in action_stack:
                    ...
                    if not block.enabled:
                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'skipped', 'message': f'Skipped (disabled)'})
                        continue
                    ...
                    if block.pre_delay_ms and block.pre_delay_ms > 0:
                        ...
                        if self._run_stop_requested(primary_tab_id):
                            ...
                            job_error = self._stop_reason(primary_tab_id)
                            break
                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'running', 'message': f'Running {block.display_name}'})
                    self._log(f'[{correlation_id}] ▶ Block {block.display_name} ({btype}) running', 'info')
                    try:
                        if btype == 'CUSTOM_FIND':
                            req = ClickRequest(selector=block.selector or 'button', label_selector=block.label_selector or '', match_text=block.match_text or '', match_mode=block.match_mode or 'contains', click_enabled=block.click_enabled, click_selector=block.click_selector or '', highlight_enabled=block.highlight_enabled, confirm_pause_ms=block.confirm_pause_ms or 700, highlight_ms=block.highlight_ms or block.highlight_duration_ms or 2000, label=block.display_name or block.name)
                            ...
                            if result == 'ok':
                                try:
                                    ...
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'FIND+CLICK ok {block.selector}', 'rect': rect_data})
                                except Exception:
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'FIND+CLICK ok {block.selector}'})
                            else:
                                fallback_list = []
                                ...
                                for fb_sel in fallback_list:
                                    ...
                                    fallback_req = ClickRequest(selector=fb_sel, label_selector='', match_text=block.fallback_text or '', match_mode='contains', click_enabled=True, click_selector='', highlight_enabled=block.highlight_enabled, confirm_pause_ms=block.confirm_pause_ms, highlight_ms=block.highlight_ms, label=f'{block.display_name} fallback {fb_sel[:30]}')
                                    result2 = await find_and_click(self.cdp, fallback_req, engine=self)
                                    if result2 == 'ok':
                                        success_fb = fb_sel
                                        break
                                if success_fb:
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Fallback ok {success_fb}'})
                                elif fallback_list:
                            ...
                            try:
                                spec = HighlightSpec(label_selector=block.label_selector or None, match_text=block.match_text or None, match_mode=block.match_mode or 'contains', color=block.color or '#00c853', caption=block.display_name or block.selector[:30], highlight_ms=block.highlight_ms or block.highlight_duration_ms or 2000, clear_first=True)
                                ...
                                self._log(f'[{correlation_id}] {msg}', level)
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success' if res.get('found') else 'failed', 'message': msg, 'rect': rect})
                            ...
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Paused {dur}ms'})
                            ...
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': reason})
                            ...
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f"Baseline {baseline.get('output_count')} outputs"})
                        elif btype == 'CHECK_SECURITY':
                            if await ctrl.is_security_dialog_visible():
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'running', 'message': 'Security dialog visible — solving or waiting'})
                                self._run_state = 'paused'
                                self._pause_requested = True
                                self._emit_arena_state()
                                try:
                                    ...
                                    self._run_state = 'running'
                                    self._emit_arena_state()
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': 'Security dialog solved'})
                            else:
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': 'No security dialog'})
                            ...
                            try:
                                ...
                                if isinstance(rect, dict) and rect.get('rect'):
                                    rd = rect.get('rect')
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Highlighted {sel}', 'rect': rd})
                            except Exception as e:
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Highlight skipped: {e}'})
                            ...
                            if block.click_selector and block.click_enabled:
                                try:
                                    req = ClickRequest(selector=block.click_selector, label_selector='', match_text='', click_enabled=True, click_selector='', highlight_enabled=block.highlight_enabled, confirm_pause_ms=block.confirm_pause_ms, highlight_ms=block.highlight_ms, label=f'{block.display_name} open dialog')
                            ...
                            try:
                                ...
                                if isinstance(rect, dict) and rect.get('rect'):
                                    rd = rect.get('rect')
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'{reason}', 'rect': rd})
                            except Exception:
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'{reason}'})
                            ...
                            try:
                                ...
                                if res.get('found'):
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Attachment preview found {sel}', 'rect': res.get('rect')})
                                else:
                            ...
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Verified {vreason}'})
                        elif btype == 'SUBMIT':
                            ...
                            try:
                                ...
                                pass
                            req = ClickRequest(selector=block.selector or 'button[aria-label="Send message"]:not([disabled])', label_selector=block.label_selector or '', match_text=block.match_text or '', click_enabled=block.click_enabled, click_selector=block.click_selector or '', highlight_enabled=block.highlight_enabled, confirm_pause_ms=block.confirm_pause_ms or 700, highlight_ms=block.highlight_ms or block.highlight_duration_ms or 2000, label=block.display_name)
                            result = await find_and_click(self.cdp, req, engine=self)
                            if result != 'ok':
                                fallback_list = []
                                if block.fallback_selector:
                                ...
                                for fb_sel in fallback_list:
                                    ...
                                    fb_req = ClickRequest(selector=fb_sel, label_selector='', match_text=block.fallback_text or '', click_enabled=True, highlight_enabled=block.highlight_enabled, confirm_pause_ms=block.confirm_pause_ms, highlight_ms=block.highlight_ms, label=f'Submit fallback {fb_sel[:40]}')
                                    result2 = await find_and_click(self.cdp, fb_req, engine=self)
                                    if result2 == 'ok':
                                        success_fallback = fb_sel
                                        break
                                if success_fallback:
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Submit via fallback {success_fallback}'})
                                else:
                                    ...
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Submit via controller {reason}'})
                            else:
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Clicked {block.selector}'})
                            ...
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting' if is_await else 'running', 'message': f'{label} — watching for processing → new output, timeout {wait_timeout}ms, reload after 1st timeout'})
                            try:
                            ...
                            for wait_cycle in range(max_wait_cycles):
                                ...
                                try:
                                    if await ctrl.is_security_dialog_visible():
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': 'Captcha during generation — solving or waiting'})
                                ...
                                if wait_cycle > 0:
                                    ...
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'{label} retry {wait_cycle + 1}/{max_wait_cycles} after reload, timeout {wait_timeout}ms'})
                                ...
                                if status == 'completed':
                                        ...
                                        if expected and assoc and (assoc != expected):
                                            ...
                                            if is_gen:
                                                ...
                                                try:
                                                    ...
                                                    baseline = {'output_count': _new_baseline_tmp.get('output_count', 0), 'output_srcs': _orig_srcs, 'timestamp': _new_baseline_tmp.get('timestamp', 0)}
                                                except Exception as _e:
                                        ...
                                        try:
                                            ...
                                            if s_test and f_test and (len(f_test) > 100):
                                                ...
                                                if is_gen:
                                                    ...
                                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Download not ready but generating {gen_details}, continue waiting'})
                                                    await asyncio.sleep(2)
                                                        ...
                                                        if s2 and f2 and (len(f2) > 100):
                                                            ...
                                                            if wait_cycle == 0:
                                                                ...
                                                                try:
                                                                    _new_baseline_tmp = await ctrl.capture_baseline()
                                                                    _orig_srcs = original_old_srcs if 'original_old_srcs' in locals() and original_old_srcs else _new_baseline_tmp.get('output_srcs', [])
                                                                    baseline = {'output_count': _new_baseline_tmp.get('output_count', 0), 'output_srcs': _orig_srcs, 'timestamp': _new_baseline_tmp.get('timestamp', 0), 'new_count_after_reload': _new_baseline_tmp.get('output_count', 0)}
                                            ...
                                            if is_gen:
                                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Exception but generating, continue waiting: {e}'})
                                                await asyncio.sleep(2)
                                                continue
                                    ...
                                    if last_wait_error in ('image_above_belongs_to_previous_prompt_await_next', 'no_exact_above_found_wait_next', 'no_exact_above_found_wait_next'):
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'No exact above, awaiting next above current prompt (prev belongs to previous) — {order_check}'})
                                        is_gen, gen_details = await ctrl.is_generating()
                                        if is_gen:
                                    ...
                                    if is_gen:
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Timeout but still generating {gen_details}, reload retry {wait_cycle + 1}'})
                                    if wait_cycle == 0:
                                        ok_r, r_msg = await ctrl.reload_page()
                                ...
                                if not wait_success and wait_cycle == max_wait_cycles - 1:
                                    if is_await and (not block.required):
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Wait timeout after reload, continuing: {last_wait_error}'})
                                        wait_success = True
                                        break
                                    else:
                            ...
                            if wait_success:
                                ...
                                try:
                                    ...
                                    if isinstance(rect, dict) and rect.get('rect'):
                                        rd = rect.get('rect')
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'New output {new_src[:60]}... downloaded {(len(file_bytes) if file_bytes else 0)} bytes' if new_src else f'{label} done', 'rect': rd})
                                except Exception:
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'New output {new_src[:60]}...' if new_src else f'{label} done'})
                            else:
                                try:
                                    await ctrl.hide_watcher_overlay()
                                except Exception:
                            ...
                            if file_bytes and len(file_bytes) > 100:
                                ...
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Already downloaded {len(file_bytes)} bytes during wait'})
                            else:
                                if not new_src:
                                ...
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'running', 'message': f'Downloading after 3s delay {new_src[:60]}... — if fail and generating, return to waiting'})
                                max_dl_cycles = 2
                                dl_success = False
                                last_dl_err = ''
                                for dl_cycle in range(max_dl_cycles):
                                    ...
                                    if dl_cycle > 0:
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'running', 'message': f'Download retry {dl_cycle + 1}/{max_dl_cycles} after reload'})
                                    max_attempts = 5
                                        ...
                                        try:
                                            ...
                                            if s and f and (len(f) > 100):
                                                ...
                                                if is_gen:
                                                    ...
                                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Download not ready but generating {gen_details}, returning to waiting'})
                                    ...
                                    if is_gen:
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Download failed but still generating {gen_details}, waiting again'})
                                        status_w, data_w = await ctrl.wait_for_new_output(baseline, timeout_ms=gen_timeout, correlation_id=correlation_id, cancel_check=lambda: self._run_stop_requested(primary_tab_id))
                                        if status_w == 'completed' and data_w.get('new_src'):
                                            new_src = data_w.get('new_src')
                                        ...
                                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'waiting', 'message': f'Not generating but download failed, reload retry {dl_cycle + 1}'})
                                        ok_r, r_msg = await ctrl.reload_page()
                                        await asyncio.sleep(3)
                                        try:
                                            ...
                                            baseline = {'output_count': latest_baseline.get('output_count', 0), 'output_srcs': original_old_srcs if 'original_old_srcs' in locals() else latest_baseline.get('output_srcs', []), 'timestamp': latest_baseline.get('timestamp', 0)}
                                        except Exception as e:
                                        ...
                                        continue
                                if dl_success:
                                    self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Downloaded {len(file_bytes)} bytes {ctype} (Python direct fallback for R2, with reload retry)'})
                                else:
                            ...
                            try:
                                ...
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Valid {fmt} {im.width}x{im.height}'})
                            except Exception as e:
                                ...
                                self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Validation fallback ext {ext}, bytes {len(file_bytes)}'})
                        elif btype == 'SAVE':
                            if not file_bytes:
                            ...
                            if not ext:
                                ext = '.png'
                            out_spec = OutputSpec(suffix=suffix, preserve_format=preserve_format, overwrite=overwrite, downloaded_ext=ext, unique_template=unique_tpl)
                            ...
                            try:
                                ...
                                pass
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Saved {output_path.name} {len(file_bytes)} bytes'})
                        elif btype == 'ADVANCE':
                            img.status = ImageStatus.COMPLETED.value
                            img.error = None
                            self.state.recalculate_progress()
                            self._save_arena()
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'success', 'message': f'Advanced to completed'})
                        else:
                            self._log(f'[{correlation_id}] Unknown block type {btype}, skipping', 'warn')
                            self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'skipped', 'message': f'Unknown type {btype}'})
                    except Exception as e:
                        job_failed = True
                        job_error = str(e)
                        self._log(f'[{correlation_id}] ❌ Block {block.display_name} failed: {e}', 'error')
                        self._emit_job_action_status({'job_id': job_id, 'block': block, 'status': 'failed', 'message': f'{e}'})
    ...
    def _emit_undo_state(self):
        try:
            ...
            payload = json.dumps({'history': hist, 'index': idx, 'canUndo': can_undo, 'canRedo': can_redo, 'count': len(hist)}, ensure_ascii=False)
    ...
    def _remember_urls(self, value):
        if not isinstance(value, list):
            return
        self.state.urls = [UrlRow(id=u.get('id', f'url_{i}'), url=u.get('url', ''), enabled=u.get('enabled', True), last_status=u.get('status', 'unchecked'), last_checked=u.get('last_checked'), error=u.get('last_error') or u.get('error')) for i, u in enumerate(value)]
        self._save_arena()
    ...
    def _remember_prompt(self, value):
        if isinstance(value, str):
            ...
            value = value.get('template') or value.get('user_prompt') or ''
            self.state.prompt['user_prompt'] = value
        else:
            return
        self._save_arena()
    ...
    def _remember_arena(self, value):
        ...
        try:
            if 'urls' in value and isinstance(value['urls'], list) and value['urls'] and ('url' in value['urls'][0]):
                self.state.urls = [UrlRow(id=u.get('id'), url=u.get('url'), enabled=u.get('enabled', True), last_status=u.get('status', 'unchecked'), error=u.get('last_error')) for u in value['urls']]
            if 'folder' in value:
    ...
    def _remember_global_edit(self, kind: str, value):
        try:
            handlers = {'grid': self._remember_grid, 'window_states': self._remember_window_states, 'urls': self._remember_urls, 'folder': self._remember_folder, 'queue': self._remember_queue, 'prompt': self._remember_prompt, 'settings': self._remember_settings, 'action_blocks': self._remember_action_blocks, 'arena': self._remember_arena}
            h = handlers.get(kind)
            if h:
                h(value)
        except Exception as e:
    ...
    def _undo_window_states(self, value):
        if isinstance(value, dict):
            self.config.set_state(window_states=value)
            self._log(f'↩ Undo window states', 'info')

    def _undo_urls(self, value):
        if not isinstance(value, list):
            return
        self.state.urls = [UrlRow(id=u.get('id', f'url_{i}'), url=u.get('url', ''), enabled=u.get('enabled', True), last_status=u.get('status', 'unchecked'), last_checked=u.get('last_checked'), error=u.get('last_error') or u.get('error')) for i, u in enumerate(value)]
        self._save_arena()
    ...
    def _undo_folder(self, value):
        if isinstance(value, dict):
            self.state.folder = value
            self._save_arena()
            self._log(f'↩ Undo folder', 'info')

    def _undo_queue(self, value):
        if not isinstance(value, list):
            return
    ...
    def _undo_action_blocks(self, value):
        if not isinstance(value, list):
            return
        try:
            self.config.set_state(action_blocks=value)
    ...
    def _apply_undo_entry(self, entry):
        ...
        try:
            handlers = {'grid': self._undo_grid, 'window_states': self._undo_window_states, 'urls': self._undo_urls, 'folder': self._undo_folder, 'queue': self._undo_queue, 'prompt': self._undo_prompt, 'settings': self._undo_settings, 'action_blocks': self._undo_action_blocks, 'arena': self._undo_arena}
            h = handlers.get(kind)
            if h:
                h(value)
            else:
    ...
    def _undo_empty_urls(self):
        self.state.urls = []
        self._save_arena()
        self._log('↩ Undo urls → empty', 'info')

    def _undo_empty_folder(self):
        self.state.folder = {'root_path': '', 'supported_types': ['.png', '.jpg', '.jpeg', '.webp'], 'ignore_ai_suffix': True}
        self._save_arena()
        self._log('↩ Undo folder → empty', 'info')

    def _undo_empty_prompt(self):
        self.state.prompt['user_prompt'] = ''
        self._save_arena()
        self._log('↩ Undo prompt → empty', 'info')

    def _undo_empty_generic(self, kind):
        self._log(f'↩ Undo {kind} → empty', 'info')

    def _undo_empty_window_states(self):
        self.config.set_state(window_states={'closed': [], 'minimized': []})
        self._log('↩ Undo window states → empty', 'info')

    def _handle_undo_empty_dispatch(self, result, kind):
        handlers = {'grid': self._undo_empty_grid, 'window_states': self._undo_empty_window_states, 'urls': self._undo_empty_urls, 'folder': self._undo_empty_folder, 'prompt': self._undo_empty_prompt}
        h = handlers.get(kind)
        if h:
            h()
        elif kind in ('queue', 'settings'):
            self._undo_empty_generic(kind)
        else:
            self._log(f"↩ Undo {kind or result.get('kind')} → empty", 'info')

    def _handle_undo_empty(self, result, undone_kind):
        try:
            self._handle_undo_empty_dispatch(result, undone_kind)
        except Exception as e:
            log.warning(f'undo empty handling failed: {e}')

    def _handle_undo_history(self, hist, idx, result):
    ...
    def undo(self):
        ...
        if idx == -1:
            ...
            self._handle_undo_empty(result, undone_kind)
        else:
            self._handle_undo_history(hist, idx, result)
        self._emit_undo_state()
        return json.dumps(result, ensure_ascii=False)
    ...
    def _emit_highlight_demo(self):
        duration = self.config.get_state('highlight_duration', 3)
        rect = {'x': 200, 'y': 200, 'width': 320, 'height': 180, 'duration': duration, 'label': 'Clicked element'}
        self.highlight_rect.emit(json.dumps(rect))

    def _init_bg_sync_primitives(self):
        import threading
    ...
    def _create_bg_loop_thread(self):
        import asyncio
        import threading
        try:
    ...
    def _make_coro_callback(self, future):
    ...
    def _schedule_on_loop(self, coro, loop):
        import asyncio
        ...
        try:
            ...
            pass
        future.add_done_callback(self._make_coro_callback(future))
        return future

    def _schedule_fallback_thread(self, coro):
        import asyncio
        import threading
    ...
    def _schedule_coro(self, coro):
        ...
        try:
            ...
            if loop and loop.is_running():
                return self._schedule_on_loop(coro, loop)
            return self._schedule_fallback_thread(coro)
        except Exception as e:
    ...
    def _report_auto_plan(self, plan, revived: int, stale: list, source: str):
        ...
        if plan.connect or revived or stale or plan.remove:
            self._emit_pool_status()
        self._log(f'🤖 Auto-connect: +{len(plan.add)} rows, {len(plan.claim)} linked, {len(plan.connect)} joined, {revived} revived, {len(stale)} stale, {len(plan.remove)} removed', 'info')
    ...
    def _is_already_connected_to(self, ws_url: str) -> bool:
        try:
    ...
    def _log_connect_failure(self, ws_url: str):
    ...
    async def _resolve_pool_tab_info(self, ws_url: str, tab_id: str):
        title = getattr(self.cdp, '_current_title', '') or ''
        url = getattr(self.cdp, '_current_url', '') or ''
        if not url or not title:
            live_title, live_url = await self._resolve_tab_info(tab_id, ws_url)
    ...
    def _reuse_existing_pool_client(self, ws_url: str, tab_id: str, title: str, url: str) -> bool:
        from app.browser.page_status import PageInfo
        existing_client, _ = self._page_pool.get_clients(tab_id)
        if not (existing_client and getattr(existing_client, 'is_connected', False)):
            return False
    ...
    async def _register_new_dedicated_client(self, ws_url: str, tab_id: str, title: str, url: str) -> bool:
        from app.browser.page_status import PageInfo
        from app.browser.cdp_arena import CDPArenaController
        from app.browser.cdp_client import CDPClient
    ...
    def _register_fallback_pool_client(self, ws_url: str, tab_id: str, title: str, url: str):
        from app.browser.page_status import PageInfo
        from app.browser.cdp_arena import CDPArenaController
        info = PageInfo(tab_id=tab_id, ws_url=ws_url, title=title, url=url)
        self._page_pool.add_page(info)
    ...
    async def _add_dedicated_pool_client(self, ws_url: str, tab_id: str, title: str, url: str):
        if self._reuse_existing_pool_client(ws_url, tab_id, title, url):
            return
        ok = await self._register_new_dedicated_client(ws_url, tab_id, title, url)
        if not ok:
            self._register_fallback_pool_client(ws_url, tab_id, title, url)

    async def _handle_pool_after_connect(self, ws_url: str):
        try:
            if not self._page_pool:
                return
            import re as _re
            m_id = _re.search('/devtools/page/([^/]+)$', ws_url)
            tab_id = m_id.group(1) if m_id else getattr(self.cdp, '_current_tab_id', '') or ws_url
            title, url = await self._resolve_pool_tab_info(ws_url, tab_id)
            await self._add_dedicated_pool_client(ws_url, tab_id, title, url)
            self._restore_page_state(tab_id)
        except Exception as e:
    ...
    async def _do_connect_tab(self, ws_url: str):
        ...
        try:
            if self._is_already_connected_to(ws_url):
            ...
            if ok:
                ...
                await self._handle_pool_after_connect(ws_url)
            else:
                self._log_connect_failure(ws_url)
        except Exception as e:
            import traceback
    ...
    def save_arena_preset(self, name: str):
        try:
            js_state = self._arena_to_js()
            cdp_cfg = {'host': self.config.get_state('cdp_host', '127.0.0.1'), 'port': self.config.get_state('cdp_port', 9222), 'user_data_dir': self.config.get_state('cdp_user_data_dir', 'C:\\arena-images-chrome'), 'extra_args': self.config.get_state('cdp_extra_args', '')}
            try:
                ...
                action_blocks = []
            doc = {'name': name, 'urls': js_state.get('urls', []), 'folder': js_state.get('folder', {}), 'prompt': js_state.get('prompt', {}), 'settings': js_state.get('settings', {}), 'images': js_state.get('images', []), 'cdp': cdp_cfg, 'action_blocks': action_blocks, 'cooldown': {'enabled': self.config.get_state('cooldown_enabled', True), 'min_seconds': self.config.get_state('cooldown_min_seconds', 300), 'captcha_penalty_seconds': self.config.get_state('cooldown_captcha_penalty_seconds', 900), 'rate_limit_penalty_seconds': self.config.get_state('cooldown_rate_limit_penalty_seconds', 1800)}, 'updated_at': datetime.utcnow().isoformat() + 'Z', 'app_version': 'arena-1.0'}
    ...
    def load_arena_preset(self, name: str):
        try:
            ...
            if not doc:
                return json.dumps({'ok': False, 'error': 'not found'})
            if 'urls' in doc:
                self.state.urls = [UrlRow(id=u.get('id', f'url_{i}'), url=u.get('url', ''), enabled=u.get('enabled', True), last_status=u.get('status', 'unchecked'), error=u.get('last_error')) for i, u in enumerate(doc.get('urls', []))]
            if 'folder' in doc:
            ...
            if 'cooldown' in doc and isinstance(doc['cooldown'], dict):
                try:
                    from app.core.cooldown import clamp_seconds
                    cd = doc['cooldown']
                    self.config.set_state(cooldown_enabled=bool(cd.get('enabled', True)), cooldown_min_seconds=clamp_seconds(cd.get('min_seconds', 300), 300), cooldown_captcha_penalty_seconds=clamp_seconds(cd.get('captcha_penalty_seconds', 900), 900), cooldown_rate_limit_penalty_seconds=clamp_seconds(cd.get('rate_limit_penalty_seconds', 1800), 1800))
    ...
    def highlight_selector(self, selector: str, color: str, duration_ms: int, caption: str):
        if not self.cdp or not self.cdp.is_connected:
            rect = {'x': 200, 'y': 200, 'width': 320, 'height': 180, 'duration': duration_ms / 1000 if duration_ms > 0 else 2, 'label': caption or selector, 'color': color}
            self.highlight_rect.emit(json.dumps(rect))
            ...
            if result_json:
                try:
                    ...
                    if data.get('found') and data.get('rect'):
                        r = data['rect']
                        rect = {'x': r.get('x', 0), 'y': r.get('y', 0), 'width': r.get('width', 100), 'height': r.get('height', 100), 'duration': (duration_ms or 2000) / 1000, 'label': caption or selector, 'color': color}
                        self.highlight_rect.emit(json.dumps(rect))
    ...
    def get_cdp_config(self):
        try:
            ...
            payload = {'host': host, 'port': int(port), 'user_data_dir': user_data_dir, 'extra_args': extra, 'url_pattern': url_pattern, 'base_url': f'http://{host}:{port}', 'is_connected': bool(self.cdp and self.cdp.is_connected), 'current_host': self.cdp._host if self.cdp else host, 'current_port': self.cdp._port if self.cdp else int(port)}
            return json.dumps(payload, ensure_ascii=False)
        except Exception as e:
    ...
    def set_cdp_config(self, config_json: str):
        try:
            ...
            self.config.set_state(cdp_host=host, cdp_port=port_i, cdp_user_data_dir=user_data_dir, cdp_extra_args=extra, url_pattern=url_pattern)
            if self.cdp:
                try:
    ...
    def get_chrome_launch_command(self):
        try:
            ...
            if extra:
                linux_cmd += f' {extra}'
            payload = {'host': host, 'port': int(port), 'user_data_dir': user_data_dir, 'extra_args': extra, 'windows': win_cmd, 'windows_with_url': win_cmd_with_url, 'linux': linux_cmd, 'test_url': f'http://{host}:{port}/json/list'}
            return json.dumps(payload, ensure_ascii=False)
        except Exception as e:
            return json.dumps({'error': str(e)}, ensure_ascii=False)
