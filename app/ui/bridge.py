...
class Bridge(QObject):
    ...
    # ---- Session recordings (docs/archive/2026-09-18-captcha-session-recording) ----
    # Same WebChannel constraint as captcha: slots live on this QObject.
    ...
    def _recording_service(self):
        ...
        if svc is None:
            ...
            svc = self._recording_service_obj = RecordingService("logs/recordings", self._log)
        return svc
    ...
    def get_recordings_list(self):
        try:
            ...
            return json.dumps(payloads.list_payload(self._recording_service()),
                              ensure_ascii=False)
        except Exception as e:
            return json.dumps({"ok": False, "error": str(e)})

    @Slot(str, result=str)
    def get_recording_detail(self, payload_json: str):
        try:
            from app.services.recording import payloads
            data = json.loads(payload_json or "{}")
            svc = self._recording_service()
            return json.dumps(payloads.detail_payload(svc.store,
    ...
    @Slot(str, result=str)
    def get_recording_snapshot(self, payload_json: str):
        try:
            from app.services.recording import payloads
            data = json.loads(payload_json or "{}")
            svc = self._recording_service()
            return json.dumps(payloads.snapshot_payload(svc.store,
                                                        str(data.get("session_id", "")),
                                                        str(data.get("name", ""))),
                              ensure_ascii=False)
        except Exception as e:
    ...
    @Slot(str, result=str)
    def get_recording_diff(self, payload_json: str):
        try:
            from app.services.recording import payloads
            data = json.loads(payload_json or "{}")
            return json.dumps(payloads.diff_payload(self._recording_service().store, data),
                              ensure_ascii=False)
        except Exception as e:
            return json.dumps({"ok": False, "error": str(e)})

    @Slot(str, result=str)
    def set_recording_label(self, payload_json: str):
        try:
            from app.services.recording import payloads
            data = json.loads(payload_json or "{}")
            svc = self._recording_service()
            result = payloads.label_payload(svc.store, str(data.get("session_id", "")),
                                            str(data.get("label", "")))
    ...
    @Slot(str, result=str)
    def delete_recording(self, payload_json: str):
        try:
            from app.services.recording import payloads
            data = json.loads(payload_json or "{}")
    ...
    def get_recording_settings(self):
        try:
            from app.services.recording import payloads
            return json.dumps(payloads.settings_payload(self._recording_service()))
        except Exception as e:
            return json.dumps({"ok": False, "error": str(e)})
