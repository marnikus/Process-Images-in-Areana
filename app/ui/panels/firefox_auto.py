...
# Config + spec builder live in `browser.uivision.config` (the dispatch lane
# shares them — services cannot import ui); re-exported so `fa.validate_config`
# and friends stay the window's API (RULE 10: one validation source).
from app.browser.uivision.config import (  # noqa: F401  (re-export)
    CONFIG_KEY, INTER_RUN_DELAY_RANGE, PAUSE_RANGE, PROFILE_LIST_KEY,
    SKIP_NO_MATCH_KEY, STORAGE_MODES, TEXT_FIELDS, TIMEOUT_RANGE,
    WAIT_TIMEOUT_RANGE, _clamp_int, _config_dir, _defaults, _validate_profiles,
    build_spec, load_config, paths_info, validate_config,
)
