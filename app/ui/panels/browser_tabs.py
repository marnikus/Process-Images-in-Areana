...
async def fetch_open_tabs(bridge):
    """Raw tab list for the reconciler — no emit, no diagnose (S6)."""
    return await bridge.cdp.fetch_tabs()


def live_deps(bridge) -> LiveDeps:
    """Wire the reconciler's four seams to this bridge (S6)."""
    return LiveDeps(
        fetch_tabs=lambda: fetch_open_tabs(bridge),
        join_tab=lambda ws: do_connect_page_pool(bridge, ws),
        commit=lambda: commit_urls(bridge, undo=False),
        log=lambda m, level="info": bridge._log(m, level),
    )
...
async def do_auto_connect_scan(bridge, source: str = "auto") -> None:
    """One reconcile pass now; pool emit on pool changes; skips when busy."""
