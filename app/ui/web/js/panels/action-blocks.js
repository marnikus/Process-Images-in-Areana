...
const ActionBlocksPanel = {
  ...
  onJobActionStatus(jobId, blockId, statusJson) {
    try {
      ...
      // Disabled per user request: rects should draw only on webpage via CDP, not in App
      // status.rect is webpage coordinates from CDP dom_highlight.py — already drawn on webpage
      // No HighlightOverlay.show() in app
      if (status.rect) {
        // optional debug log, but no app overlay
        // console.debug('Webpage rect (CDP) for', status.block_name||blockId, status.rect);
      }
      this.renderJobStack(jobId); this.renderAllJobs(); this.updateFooter();
