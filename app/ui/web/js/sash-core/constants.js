...
window.SashCoreConstants = (() => {
  const WINDOWS = [
    ...
    { id: 'arena_presets', title: 'Arena Presets' }, { id: 'live_debug', title: 'Live Worker & Queue Debug' },
  ];
  const WINDOW_IDS = WINDOWS.map(w => w.id);
  const WINDOW_TITLES = Object.fromEntries(WINDOWS.map(w => [w.id, w.title]));
  return { WINDOWS, WINDOW_IDS, WINDOW_TITLES, V1_WINDOW_IDS: WINDOW_IDS, V2_WINDOW_IDS: WINDOW_IDS, V3_WINDOW_IDS: WINDOW_IDS, VERSION: 6, MAX_DEPTH: 12, MIN_SIZE: 4 };
})();
