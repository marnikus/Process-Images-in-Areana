...
window.SashCoreConstants = (() => {
  const WINDOWS = [
    ...
    { id: 'arena_presets', title: 'Arena Presets' }, { id: 'live_debug', title: 'Live Worker & Queue Debug' }, { id: 'job_history', title: 'Job History' }, { id: 'firefox_auto', title: 'Firefox auto with Extension' }, { id: 'workspace', title: 'Global Saving System' },
  ];
  const WINDOW_IDS = WINDOWS.map(w => w.id);
  const WINDOW_TITLES = Object.fromEntries(WINDOWS.map(w => [w.id, w.title]));
  return { WINDOWS, WINDOW_IDS, WINDOW_TITLES, V1_WINDOW_IDS: WINDOW_IDS, V2_WINDOW_IDS: WINDOW_IDS, V3_WINDOW_IDS: WINDOW_IDS, VERSION: 9, MAX_DEPTH: 12, MIN_SIZE: 4 };
})();
