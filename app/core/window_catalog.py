...
WINDOWS = [
    ...
    {"id": "uivision", "title": "Firefox auto with Extension"},
]
...
GRID_VERSION = 8  # 7 → 8: the 18th window (uivision); older layouts migrate (extra leaf), never rejected
    ...
    return split("col", [
        ...
        split("row", [
            ...
            split("col", [leaf("browser"), leaf("arena_presets"), leaf("progress"),
                          leaf("watcher"), leaf("live_debug"), leaf("job_history"),
                          leaf("uivision")], [20, 15, 12, 15, 13, 13, 12]),
        ], [45, 35, 20]),
        leaf("log"),
    ], [38, 40, 22])
