...
from .output_state import flatten_diagnostics, normalize_old_keys
from .output_wait import WaitRequest, wait_for_new
...
# NOTE: baseline/output JS moved to app/browser/output_probes.py (v3).
# Old JS_BASELINE/JS_CHECK_NEW_OUTPUT deleted to avoid drift (RULE 21).
...
class CDPArenaController:
    ...
    async def wait_for_new_output(self, baseline: Dict[str, Any], timeout_ms: int = 180000, correlation_id: Optional[str] = None, cancel_check=None) -> Tuple[str, Dict[str, Any]]:
        """Poll via output_wait v3 (normalized keys, fallback)."""
        req = WaitRequest(baseline, timeout_ms, correlation_id, cancel_check)
        status, data = await wait_for_new(self.cdp, self._log, req)
        check = data.get("check") or data.get("last_check") or {}
        if check:
            data.update(flatten_diagnostics(check))
        if status == "completed" and "baseline" not in data:
            data["baseline"] = baseline
        if status == "failed" and "last_baseline" not in data:
            try:
                data["last_baseline"] = await self.capture_baseline()
            except Exception:
                pass
        return status, data
    ...
    async def get_generation_state(self, correlation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        js_check = build_check_js([], correlation_id)
        try:
