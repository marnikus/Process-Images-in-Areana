...
def _charge_pause(clock, seconds: float) -> None:
    """Charge the wait's pause clock (None = Watcher OFF, nothing rides along)."""
    if clock is not None:
        clock.note(seconds)


async def _settle_timed(settler, clock) -> None:
    """Run the settler; the seconds it took are captcha-wait seconds, charged even if it failed (S3)."""
    t0 = time.monotonic()
    try:
        await settler()
    finally:
        _charge_pause(clock, time.monotonic() - t0)
...
def _timeout_text(result: Dict[str, Any], timeout_ms: int) -> str:
    """`Timeout after Nms`, plus what the captcha pause absorbed when a clock was charged (S3)."""
    note = result.get("pause_note")
...
async def _map_wait_result(cdp, result, baseline, timeout_ms):
    ...
    return "failed", {"error": _timeout_text(result, timeout_ms), "paused_s": result.get("paused_s", 0),
