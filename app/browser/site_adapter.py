...
SELECTORS: Dict[str, SelectorObject] = {
    ...
    "processing_spinner": SelectorObject(
        ...
        fallbacks=["div.h-5.w-5.flex-shrink-0.animate-spin", "div.animate-spin > canvas"],
        scope="div.flex.min-w-0.flex-1.items-center.gap-2",
        mustBeVisible=True,
        mustBeEnabled=False,
        expectedCount=1,
        verification="spinner visible near model label indicates processing",
        evidence="spec A",
        lastVerified="2026-09-15",
    ),
    ...
    "send_button": SelectorObject(
        ...
        mustBeEnabled=True,
        expectedCount=1,
        verification="click once, confirm processing/loading state, prevent duplicate",
        evidence="Directly Chat...html + spec F",
        lastVerified="2026-09-15",
    ),
    ...
    "output_image": SelectorObject(
        ...
        scope="div.no-scrollbar",
        mustBeVisible=True,
        mustBeEnabled=False,
        expectedCount=0,  # 0..n
        verification="capture all matching nodes and src before submission, result must be newly inserted or new source after submission, appear after current prompt, finish loading, nonzero natural dimensions",
        evidence="spec G",
        lastVerified="2026-09-15",
    ),
