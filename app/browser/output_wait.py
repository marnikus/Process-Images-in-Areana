...
class WaitSpec:
    ...
    pause: Optional[PauseClock] = None  # S3: absorbed captcha-wait seconds (None = plain wall clock)
...
async def _check_timeout(state: LoopState, spec: WaitSpec, log_cb: Callable) -> dict | None:
    ...
    if fb:
        return fb
    evidence = {"paused_s": spec.pause.total, "pause_note": spec.pause.describe()} if spec.pause else {}
