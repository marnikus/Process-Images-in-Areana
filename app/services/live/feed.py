...
* `recover_stale_processing` returns crash leftovers (`processing` with no
  live tab job) to `pending` + `selected` at Start.
...
from app.core.run_scope import CLAIMABLE_STATUSES, claim_scope
