...
async def _claim_tab(ctx: BatchCtx) -> bool:
    """Refresh the run tab per image on the LIVE row set (S5); False = none usable."""
...
def pass_complete(ctx: BatchCtx) -> None:
    """Pass tail: cancelled/complete line + emit (run state is the supervisor's, D-8)."""
...
def _load_run_settings(ctx: BatchCtx, plan) -> None:
    """Images from the pass plan + prompt template from live state."""
...
async def prepare_batch(bridge, plan) -> Optional[BatchCtx]:
    """Controller + tab + settings against the plan; None when the claim fails."""
    ...
    tab_id = await resolve_and_claim_tab(bridge, getattr(bridge.cdp, "_current_tab_id", "") or "", plan.allowed)
    if not tab_id:
        bridge._log("⏳ No usable checked tab right now — the live run retries on the next wake", "warn")
        return None
    await _warn_unready(bridge, ctrl)
    ctx = BatchCtx(bridge=bridge, ctrl=ctrl, urls=list(plan.urls), allowed=set(plan.allowed), tab_id=tab_id)
