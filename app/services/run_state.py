...
def _submit_tracked(bridge, loop, coro):
    """Submit + done-callback on the live bg loop."""
...
def schedule_batch(bridge, coro):
    """Batch scheduling (S4): `schedule_coro` + the future is ALWAYS the batch future.

    Replaces the deleted name-sniffing `_track_batch_future` — callers say
    what a batch is, instead of the scheduler inspecting `cr_code.co_name`.
    """
