...
def _save_bytes(bridge, source, data: bytes) -> Path:
    ...
    return out.save_beside(source, data, out.output_spec(settings_of(bridge), ext))
...
def _suffix(bridge) -> str:
    """The configured `_AI` suffix — the one `output_spec` reads for the save."""
    return out.output_spec(settings_of(bridge), "").suffix
...
def _complete(case: _Case, path: Path) -> str:
    ...
    shutil.rmtree(job_folder(case.bridge, case.job_id), ignore_errors=True)
    say(case.bridge, f"♻ 🦊 [{case.job_id}] recovered — {path.name} saved, no resubmit", "success")
    return "completed"
...
def _review(case: _Case, reason: str) -> str:
    ...
    say(case.bridge, f"⚠ 🦊 [{case.job_id}] needs review — {message}", "warn")
    return "needs_review"
...
async def recover_firefox_jobs(bridge) -> dict:
    ...
    for record in journal.open_records():
        try:
            ...
            say(bridge, f"⚠ 🦊 [{record.get('job_id')}] recovery failed: {exc}", "warn")
            outcome = "error"
