"""The `firefox_auto` config — one validated dict for the window AND the job lane.

Moved out of `ui/panels/firefox_auto.py` (2026-09-25, design D-6) so
`services/firefox_job` can build the runner's `RunSpec` without importing
`app.ui` (panels → services only). The panel re-exports every name, so its
slots, the frozen 141-slot surface and `tests/test_firefox_auto_panel.py`
are untouched.

Owns: defaults (`session.json`'s own dict), the clamps/ranges, text-field
trimming, the macro-name grammar delegation and the paths/spec builders.
RULE 13: a stored dict heals through `validate_config`; a retired key dies
with the rebuilt defaults (RULE 10's dead-key corollary).
"""
...
def build_spec(bridge, cfg: dict):
    ...
    return RunSpec(pattern=cfg["pattern"], target=cfg["target"],
                   ...
                   pause_ms=cfg["pause_ms"], config_dir=config_dir(bridge),
