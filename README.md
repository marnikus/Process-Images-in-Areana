...
- **Browser Debug Connection (Chrome / Firefox / Edge):** one browser selector over a data-driven registry (`app/browser/browsers.py`) — shared debug host + base port + URL pattern, and per browser its own data dir, extra args, resolved endpoint (`base + offset`, so two browsers run side by side) and copy-ready launch command. Chrome/Edge speak CDP (`--remote-debugging-port`, one socket per tab). Firefox is attached over the **DevTools RDP socket** (`--start-debugger-server`, one socket for the whole browser): start Firefox yourself, the app attaches for tab list / JS / click-only actions and detaches again without a restart, and nothing it generates ever sets `navigator.webdriver` (the Remote Agent `--remote-debugging-port` does — that channel is only a detected fallback). Its profile prefs are shown as pasteable `user.js` lines with an opt-in **Prepare Profile** button (your own `user.js` lines are kept; a plain Save never writes into your profile). Each row lists what its protocol supports and names the gaps (`⛔ not in RDP: dom, input, screenshot, set_files`) instead of timing out. Every payload rides the existing CDP slots — the bridge slot surface is unchanged. **Every enabled browser is listed in one pass**, each at its own endpoint (`base + offset`: Chrome 9223, Firefox 9224, Edge 9225 …) and over its own protocol: a browser switched off in Settings is skipped, one that is down is reported once — with the flag that opens it — never once per pass, and never hides a live one. Connect works on both channels: pick any listed tab and the app attaches to it (Firefox tabs ride the DevTools socket as `rdp://host:port/ctx-N`), the job path's `client.evaluate(...)` runs in that tab, and detaching leaves the browser running. An endpoint that answers HTTP but not its DevTools socket is explained instead of retried forever — that is the Remote Agent opened by `--remote-debugging-port`, which sets `navigator.webdriver` (Bug 1719505). Firefox starts with **your own profile** by default (leave the dir empty) — real history, cookies and extensions — and `-no-remote` is what makes the debug socket open while Firefox is running. Two facts worth knowing: the URL-bar robot icon (`reason: DevTools`) is Firefox's own cue for *any* running DevTools server, it is browser chrome no website can read and no pref removes it while the socket is open; and the `Allow connection?` dialog is `devtools.debugger.prompt-connection` — click Allow once, or let **Prepare Profile** write it into the profile Firefox is actually running (the app opens one connection per pass, so the dialog cannot loop). What a site *can* read is measured, not claimed: **Diagnose** prints `navigator.webdriver=false` from your own tab. Firefox is asked for permission **once**: the app keeps one DevTools socket per browser for the whole run instead of dialling again for every listing and every evaluate, it waits for you to click Allow instead of retrying every second, and after the first Allow it switches the dialog off in the running browser (`devtools.debugger.prompt-connection`) so it does not come back — ever. If Firefox is quiet, the panel says so once and stops asking; **Reparse** is the button that tries again. A browser that did not answer also keeps its URL rows, so a parked or closed Firefox never looks like one with no tabs.
