...
from app.browser.cdp import client as cdp_impl, probe, tabs as cdp_tabs  # C2 real owners
...
def test_fetch_tabs_sync_dedupes_and_prefers_host(monkeypatch):
    monkeypatch.setattr(cdp_tabs, "_is_port_open", lambda h, p, timeout=0.8: True)
    ...
    def fake_fetch(url, timeout=3.0):
        ...
        for t in RAW_TABS:
            ...
            tabs.append(t)
        return tabs, ""
    monkeypatch.setattr(cdp_tabs, "_fetch_json_sync", fake_fetch)
...
def test_fetch_tabs_sync_port_closed_reports_error(monkeypatch):
    monkeypatch.setattr(cdp_tabs, "_is_port_open", lambda h, p, timeout=0.8: False)
    tabs, err, tried = fetch_tabs_sync(host="127.0.0.1", port=9222)
...
def test_fetch_tabs_sync_bad_json(monkeypatch):
    monkeypatch.setattr(cdp_tabs, "_is_port_open", lambda h, p, timeout=0.8: True)
    monkeypatch.setattr(cdp_tabs, "_fetch_json_sync",
...
def test_diagnose_sync_branches(monkeypatch):
    # probe.py binds the helpers at import: patch ITS names, not cdp.tabs'
    monkeypatch.setattr(probe, "_is_port_open", lambda h, p, timeout=1.0: False)
    monkeypatch.setattr(probe, "_fetch_json_sync",
                        lambda url, timeout=3.0: (None, "refused"))
    # 1) no open ports
    monkeypatch.setattr(cdp_tabs, "_is_port_open", lambda h, p, timeout=1.0: False)
    monkeypatch.setattr(cdp_tabs, "_fetch_json_sync",
                        lambda url, timeout=3.0: (None, "refused"))
    out = diagnose_sync(host="127.0.0.1", port=9222)
    assert out["summary"].startswith("❌") and out["tabs"] == []
    # 2) open but zero tabs
    monkeypatch.setattr(probe, "_is_port_open", lambda h, p, timeout=1.0: True)
    monkeypatch.setattr(probe, "_fetch_json_sync",
...
async def test_highlight_and_clear_use_evaluate(cdp_server):
    def responder(method, params, server):
        ...
        return {"ok": True}, None
    client = await _connected(cdp_server, responder)
    hit = await client.highlight_element(
        "#gen", HighlightSpec(color="#00ff00", duration_ms=300, caption="go"))
...
async def test_fetch_tabs_falls_back_to_sync(monkeypatch, cdp_server):
    ...
    fallback = []
    monkeypatch.setattr(cdp_impl, "fetch_tabs_sync",
...
def test_tab_wrappers_delegate_to_pure():
    found = cdp_tabs._parse_tabs(RAW_TABS, preferred_host="127.0.0.1", preferred_port=9222)
    assert isinstance(found[0], TabInfo) and found[0].id == "t1"
    assert all(not t.url.startswith("chrome://newtab") for t in cdp_tabs._filter_real_tabs(found))
    norm = cdp_tabs._normalize_ws_url("ws://10.0.0.9:9222/devtools/page/t1", "127.0.0.1", 9222)
    assert norm.startswith("ws://127.0.0.1:9222")
    assert cdp_tabs._is_devtools_url("devtools://devtools/bundled/inspector.html") is True
