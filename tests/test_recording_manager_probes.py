...
class TestFacade:
    def test_list_count_delete_labels(self, tmp_path):
        ...
        row = mgr.set_labels(rec.session_id, "manual", "passed")
        assert row["actor_label"] == "manual" and row["result_label"] == "passed"
    ...
    def test_finish_logs_the_outcome_and_drops_the_recorder(self, tmp_path):
        mgr, ctrl, _, logs = make_manager(tmp_path)

        async def scenario():
            # one loop: the recorder's listener handles bind to the running loop
            rec = await mgr.start(ctrl, {"tab": "T1", "eid": "e1"})
            assert rec is not None
            await mgr.finish(rec, SolveOutcome(status="solved", method="auto"))

        asyncio.run(scenario())
        assert mgr._active == {}
        assert any("finished" in message for _, message in logs)

    def test_compare_sessions_uses_the_stored_read_models(self, tmp_path):
        mgr, ctrl, _, _ = make_manager(tmp_path)

        async def scenario():
            left = await mgr.start(ctrl, {"tab": "T1", "eid": "e1"})
            await mgr.finish(left, SolveOutcome(status="solved", method="auto"))
            right = await mgr.start(ctrl, {"tab": "T2", "eid": "e2"})
            await mgr.finish(right, SolveOutcome(status="auto_failed", method="manual"))
            return left, right

        left, right = asyncio.run(scenario())
        report = mgr.compare_sessions(left.session_id, right.session_id)

        assert report["left"]["session_id"] == left.session_id
        assert report["right"]["session_id"] == right.session_id
        assert "common" in report and "first_divergence" in report
...
class TestProbes:
    """The captcha probes are their own page agents (install/drain/snapshot/stop);
    tests/js/test_captcha_recording.mjs executes the same sources in a real DOM."""
    ...
    def test_install(self):
        ...
        assert "reused" in js  # idempotent install guard
