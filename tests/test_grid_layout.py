...
def test_preset_save_load_round_trip(isolated_config_dir):
    ...
    assert doc["grid"]["window_count"] == 19  # 2026-09-25: + workspace window
    assert len(leaf_ids(json.loads(doc["grid"]["payload"])["tree"])) == 19  # + workspace
    assert len(leaf_ids(doc["grid"]["tree"])) == 19  # + workspace
    ...
    assert len(leaf_ids(loaded["grid"]["tree"])) == 19  # + workspace
