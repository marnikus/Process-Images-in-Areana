...
def test_preset_save_load_round_trip(isolated_config_dir):
    ...
    assert doc["grid"]["window_count"] == 18
    assert len(leaf_ids(json.loads(doc["grid"]["payload"])["tree"])) == 18
    assert len(leaf_ids(doc["grid"]["tree"])) == 18
    ...
    assert len(leaf_ids(loaded["grid"]["tree"])) == 18
