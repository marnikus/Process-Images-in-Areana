...
def test_preset_save_load_round_trip(isolated_config_dir):
    ...
    assert doc["grid"]["window_count"] == 17
    assert len(leaf_ids(json.loads(doc["grid"]["payload"])["tree"])) == 17
    assert len(leaf_ids(doc["grid"]["tree"])) == 17
    ...
    assert len(leaf_ids(loaded["grid"]["tree"])) == 17
