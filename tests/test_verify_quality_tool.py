...
PROBE_REL = "app/ui/panels/_probe_tmp.py"


def _write_funcs_probe(funcs: dict):
    """Synthetic probe file: _probe_big (35 LOC) + _probe_new (31 LOC)."""
...
def _bridge_baseline_with_funcs(tmp_path: Path, funcs: dict) -> Path:
    """Baseline entry for the synthetic probe file (W1.6: cdp_panel funcs went compliant)."""
    _write_funcs_probe(funcs)
    entry = {"max_func_loc": max(funcs.values()), "max_class_loc": 0,
             "func_count": len(funcs), "funcs": funcs}
    base = tmp_path / "funcs_baseline.json"
    base.write_text(json.dumps({PROBE_REL: entry}), encoding="utf-8")
    return base
...
def test_legacy_new_symbol_fails_with_funcs_map(tmp_path: Path):
    ...
    legacy file and must fail (F-18/D6: adding a 31-LOC func fails)."""
    # _probe_big (35 LOC) is grandfathered; _probe_new (31 LOC) is unknown
    try:
        base = _bridge_baseline_with_funcs(tmp_path, {"_probe_big": 35})
        result = run_tool("--json", "--allow-legacy", "--baseline", str(base))
        breaches = [b for b in json.loads(result.stdout)["breaches"]
                    if b["file"] == PROBE_REL and b.get("metric") == "loc"]
        known = [b for b in breaches if b["name"] == "_probe_big"]
        new = [b for b in breaches if b["name"] == "_probe_new"]
        ...
        (ROOT / PROBE_REL).unlink(missing_ok=True)
...
def test_legacy_symbol_growth_fails_with_funcs_map(tmp_path: Path):
    ...
    try:
        base = _bridge_baseline_with_funcs(tmp_path, {"_probe_big": 5})
        result = run_tool("--json", "--allow-legacy", "--baseline", str(base))
        breaches = [b for b in json.loads(result.stdout)["breaches"]
                    if b["file"] == PROBE_REL and b.get("metric") == "loc"
                    and b["name"] == "_probe_big"]
