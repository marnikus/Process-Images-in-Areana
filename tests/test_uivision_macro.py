...
JS-level mouse command, and the framework test macro is the official demo
shape — open URL → bringBrowserToForeground → pause → XClick → echo done —
with the per-run values riding `${!cmd_var1}` / `${!cmd_var2}`.
"""
...
def test_framework_macro_is_the_demo_sequence():
    ...
    assert commands[0]["Target"] == macro.URL_VAR      # ${!cmd_var1} — the URL
    assert commands[2]["Target"] == "2500"
