...
class TestCategory:
    """Endpoint classes the comparison view groups by (never query strings)."""

    def categories(self):
        c, calls, _ = collector()
        urls = {
            "captcha": "https://www.google.com/recaptcha/api2/anchor",
            "verification": "https://arena.ai/challenge/verify",
            "generation": "https://arena.ai/generate/image",
            "page_api": "https://arena.ai/api/jobs?token=SECRET",
            "other": "https://arena.ai/about",
        }
        for request_id, url in enumerate(urls.values()):
            c.on_event(ev("Network.requestWillBeSent", requestId=str(request_id), type="Fetch",
                          request={"url": url, "method": "POST"}))
        asyncio.run(c.drain())
        return {kind: payload["category"] for kind, payload, _ in calls}

    def test_buckets(self):
        got = self.categories()
        assert got["network_request"] in ("captcha", "verification", "generation", "page_api",
                                          "other")

    def test_each_branch_is_reachable(self):
        from app.services.captcha_recording.network import _category
        assert _category("https://www.google.com/recaptcha/api2/anchor") == "captcha"
        assert _category("https://arena.ai/challenge/verify") == "verification"
        assert _category("https://arena.ai/generate/image") == "generation"
        assert _category("https://arena.ai/api/jobs?token=SECRET") == "page_api"
        assert _category("https://arena.ai/about") == "other"
