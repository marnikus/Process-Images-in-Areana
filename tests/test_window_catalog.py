"""S8 · one window contract, 16 windows, and the L-5 rescue (I-51).
...
away), `GRID_VERSION` is 6 and stored v5 layouts migrate.
...
def test_ids_and_titles_have_no_duplicates_and_no_legacy_names():
    assert len(wc.WINDOW_IDS) == 16 == len(set(wc.WINDOW_IDS))
    assert len(set(wc.WINDOW_TITLES.values())) == 16
...
def test_grid_version_is_six_and_v5_layouts_migrate():
    assert wc.GRID_VERSION == 6
    v5_tree = json.loads(json.dumps(wc.default_grid_tree()))
    # drop the live_debug leaf → the shape a v5 file on disk has (15 leaves)
    def strip(node):
        if node.get("t") != "split":
            return node
        keep = [i for i, k in enumerate(node["children"]) if not (k.get("t") == "leaf" and k["id"] == "live_debug")]
    ...
    v5_tree = strip(v5_tree)
    assert "live_debug" not in layout_service.leaf_ids(v5_tree)
    payload, err = layout_service.canonical_grid_payload(json.dumps({"v": 5, "tree": v5_tree}))
    assert err is None
    doc = json.loads(payload)
    assert doc["v"] == 6 and sorted(layout_service.leaf_ids(doc["tree"])) == sorted(wc.WINDOW_IDS)
    v6, err = layout_service.canonical_grid_payload(layout_service.default_payload())
    assert err is None and layout_service.canonical_grid_payload(v6) == (v6, None)  # canonical v6 is a fixed point
    assert layout_service.canonical_grid_payload(json.dumps({"v": 7, "tree": wc.default_grid_tree()}))[1] == "unsupported version 7"
