...
class TestCohortLabels:
    ...
    def test_result(self):
        ...
        assert _result({"result_label": "mixed"}) == "mixed"
...
class TestCohort:
    def test_empty(self):
        ...
        assert c["matrix"]["bot"] == {"passed": 0, "failed": 0, "mixed": 0, "unknown": 0}
        assert c["matrix"]["manual"] == {"passed": 0, "failed": 0, "mixed": 0, "unknown": 0}
...
class TestReader:
    ...
    def test_events_missing(self, tmp_path):
        assert EvidenceReader._events(tmp_path / "nope.jsonl") == []
    ...
    def test_snapshot_non_dict(self, tmp_path):
        ...
        with gzip.open(snap / "000001.json.gz", "wt", encoding="utf-8") as h:
            json.dump([1, 2], h)
        assert EvidenceReader._snapshot(snap) == {}
