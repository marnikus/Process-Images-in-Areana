...
class TestRecordingStore:
    ...
    def test_set_labels(self, tmp_path):
        s = self.make(tmp_path)
        m = s.create({})
        row = s.set_labels(m["session_id"], "bot", "passed")
        assert row["result_label"] == "passed" and row["actor_label"] == "bot"
        try:
            s.set_labels(m["session_id"], "bot", "nope")
            assert False, "expected ValueError"
        except ValueError:
            pass
    ...
    def test_recover_interrupted(self, tmp_path):
        ...
        manifest = read_manifest(s1.session_folder(m["session_id"]))
        manifest.update({"status": "recording", "ended_at": ""})
        write_manifest(s1.session_folder(m["session_id"]), manifest)
...
class TestModels:
    ...
    def test_labels(self):
        assert VALID_LABELS == {"unknown", "bot", "manual", "mixed"}  # actor axis
        assert VALID_RESULT_LABELS == {"unknown", "passed", "failed"}  # result axis
