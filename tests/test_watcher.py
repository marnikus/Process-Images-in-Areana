...
def test_start_without_loop_defers_task_and_update_config_toggles():
    ...
    assert svc._task is None and svc._running is True
    svc.update_config(enabled=False)  # stop transition
    assert svc._running is False and svc.state.status == "idle"
    assert not WatcherService(config=WatcherConfig(enabled=False))._running
