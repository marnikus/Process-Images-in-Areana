...
async def test_security_announces_while_solving(tmp_path, monkeypatch):
    ...
    bridge = make_bridge(session={"watcher_enabled": True})  # S2: ON path intended
...
async def test_settle_and_note_stamps_policy(tmp_path, monkeypatch):
    ...
    ctx = make_ctx(make_bridge(session={"watcher_enabled": True}), ctrl, make_client(), make_img(tmp_path))
