...
Subprocess against tmp_path fixtures via the gate's --root flag (ported from
Area D onto this branch's single B+C gate):
  1. 31-LOC function in a baseline file (regression)      -> verify_quality fails
  2. 40-LOC JS function (new symbol)                      -> JS lane fails
...
def _run_baseline(root: Path) -> None:
    """Record the tmp repo's baseline with the gate's own integrator flag."""
    subprocess.run([sys.executable, str(VERIFY), "--root", str(root), "--record-baseline"],
...
def test_grown_function_in_baseline_file_fails(repo: Path):
    ...
    assert "ratchet" in res.stdout and "grew" in res.stdout
...
def test_per_file_coverage_drop_fails(repo: Path):
    ...
    res = _gate(repo, "--changed-files", "app/pkg/mod_a.py", "--allow-legacy")
    assert res.returncode == 1
    assert "coverage dropped" in res.stdout
...
def test_40loc_js_function_fails(repo: Path):
    """The JS lane of the single gate must reject a 40-LOC function."""
    (repo / "app" / "ui" / "web" / "js" / "a.js").write_text(JS_40)
    res = _gate(repo, "--changed-files", "app/ui/web/js/a.js")
    assert res.returncode == 1, res.stdout + res.stderr
    assert "max_func_loc 40 > 30" in res.stdout
