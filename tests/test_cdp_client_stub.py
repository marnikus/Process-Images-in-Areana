...
from app.browser.cdp.connection import ws_candidates, ws_tab_id
...
async def test_highlight_element_via_stub(stub):
    client = CDPClient("127.0.0.1", stub.port)
    await client.connect(stub.page_ws_url)
    res = await client.highlight_element("div.x", "#00AA00", 1500, "cap")
...
def test_ws_candidates_dedupe_and_variants():
    ...
    cands = ws_candidates(url, "ABC", "127.0.0.1", 9222)
    assert cands[0] == url
    assert len(cands) == len(set(cands))
    assert "ws://127.0.0.1:9222/devtools/page/ABC" in cands
    assert ws_tab_id(url) == "ABC"
    assert ws_tab_id("ws://127.0.0.1:9222/devtools/browser/x") == ""
