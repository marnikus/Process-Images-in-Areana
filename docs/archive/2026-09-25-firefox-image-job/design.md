...
* **S3** native dialog: XType path + `${KEY_ENTER}` accepted (2019 forum note: sometimes needs XClick "Open"). **Failed live 2026-09-26** (owner: dialog open, File name empty, dialog never closed). Fixed in `uivision/file_dialog.py`: `!StringEscape` off → quoted native path into `!clipboard` → `XDesktopAutomation true` → Alt+N, Ctrl+A, Ctrl+V, Alt+O → browser mode; Enter (other-locale fallback) and ESC only while `document.hasFocus()` is false and our preview is absent. Still to confirm live: Alt+O on the owner's Windows locale (the Enter fallback covers a miss). **Second live report 2026-09-26:** the + now opens a popup (“Add files — Up to 15MB per file”, paperclip) and only that item opens the dialog → new selector `add_files_menu_item`, `job_scripts.upload_item_js` tags it, gated RED rect + XClick `css=[data-arena-upload-item]` before the dialog rows. **Third live report 2026-09-26:** the dialog showed `"config/uivision/uploads/arena_….png"` — “The file name is not valid” (a relative staging path) → D-4 amended: no staging copy, the dialog gets the queue file's absolute native path and the preview must carry its name; and `${arenaJob}` reaches executeScript as a JSON string (`.data is undefined`) → the ESC / send-guard flags parse a string or an object, an unreadable reply never clicks send.
