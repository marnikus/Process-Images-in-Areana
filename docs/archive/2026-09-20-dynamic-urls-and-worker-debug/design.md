...
| 02 | Captcha ⇄ timeout | Watcher **OFF** ⇒ the generation timeout runs uninterrupted and no captcha work happens (round-1 D-1). Watcher **ON** ⇒ the per-page generation timeout is **paused** for as long as that page waits for a captcha to clear, with or without a 2Captcha key; without a key the app detects, says so honestly, and waits for the manual solve. |
...
## 2. Decisions (D-11…D-22; amendments named explicitly)
...
| **D-13** *(amends round-1 D-1)* | Watcher **ON** ⇒ the generation wait charges every second spent inside the captcha settle to a **`PauseClock`**, and `_check_timeout` subtracts it: `elapsed = now − start − paused`. Watcher **OFF** ⇒ the clock stays at 0 because the `security_settler` is never installed (round-1 D-1) ⇒ the timeout runs exactly as today. | Brief item 02 bullets 1-2, and evidence §2: today an unbounded RULE-20 wait (`wait_captcha_cleared` "never gives up") always ends in `Timeout after …ms` even after the dialog cleared. One subtraction in one function fixes it for every caller of the wait loop. | (a) Restarting the wait after a settle (re-baselines `old_srcs`, risks losing the output that appeared during the solve — RULE 15). (b) Raising `watcher_generation_timeout_sec` globally (punishes the no-captcha case). (c) A per-page deadline in the pool (two owners of one timeout). |
| **D-14** | The pause is **unbounded but observable**: Stop/abort is honoured inside the wait (unchanged), a throttled line reports the paused total every 60 s, the pool row shows `waiting_captcha`, and the debug window shows the paused worker with its wait duration. `watcher_captcha_timeout_sec` stays a **warning/overlay** number, never a kill switch. | RULE 20 (wait-only) + I-34; a hidden unbounded wait is the real trap, so it is surfaced instead of capped. The user asked for "timeout STOPS while waiting for captcha solve" — a cap would re-introduce the failure. | Killing the job at `watcher_captcha_timeout_sec` (contradicts item 02 and RULE 20); capping the pause at N minutes (same). |
...
| `app/browser/output_wait.py` | existing (211 → ~245) | gains `PauseClock` (mutable `total`, `note(seconds)`), `paused_elapsed(start, clock)` and `describe()` — the pause term lives with the timeout it modifies, and `cdp_arena/output.py:13` already imports `WaitSpec` from here | +34 |
...
| `app/services/captcha/policy.py` | round-1 file | gains `wait_reason(bridge) -> str` (three-way, D-15) and `has_key(bridge) -> bool` | +20 |
...
```python
# app/browser/output_wait.py (added next to WaitSpec, ~34 LOC)
@dataclass
class PauseClock:
    total: float = 0.0
    def note(self, seconds: float) -> None:      # ignore <= 0, never raises
    def describe(self) -> str:                   # "+37s captcha wait" or ""
...
### 5.2 The three cases after W1

| Case | Detection | Wait | Generation timeout | User-visible wording |
|---|---|---|---|---|
| Watcher **OFF** | none (round-1 D-1: `check_security` returns False, `handle_captcha` → `out_of_scope`, no settler installed) | none | **runs** (clock 0) | the gate that actually broke (`WAIT_OUTPUT` timeout / page-error text) |
| Watcher **ON** + key | detect probe + stats + recording + `waiting_captcha` row | `_manual_wait` until the solver or the user clears it | **paused** for the whole wait | overlay reason: *"Captcha Watcher is solving it (2Captcha SDK)"*; throttled `⏸ Generation timeout paused for captcha wait (Ns)` |
| Watcher **ON**, no key | identical (detection does not depend on the key) | identical — waits for the **manual** solve | **paused** | overlay reason (D-15): *"Watcher ON, no 2Captcha key — solve it in Chrome; this job's timeout is paused"*; the existing `solver_start` warn line stays (`watcher_solver.py:130-134`) |

`policy.wait_reason(bridge)` is the single owner of that wording; `_resolve_captcha`
(`service.py:232-239`, span 8 → 9) calls it and passes the result to `_manual_wait`, which is
**untouched** (zero headroom, evidence §6.1).
...
* Cooldown penalty recording (`_record_penalty`, once per cleared edge) is unchanged — the pause does
  not create a second captcha event.
...
| 4 | **PauseClock + timeout pause** (item 02) | `browser/output_wait.py` (`PauseClock` + `paused_elapsed`), `browser/cdp_arena/output.py` | `tests/test_pause_clock.py`, `tests/test_output_wait_timeout_pause.py`, existing `output_wait` suite (94.95 % floor) |
| 5 | **Captcha scope + wait reason** (item 02, round-1 feature 01) | new `services/captcha/policy.py`; `services/captcha/service.py`; `services/single_job_runner.py` | `tests/test_captcha_scope.py`, `tests/test_captcha_wait_reason.py`, goldens unchanged (harness arms Watcher ON) |
...
| **I-44** | A page's generation timeout is paused for exactly the time that page spends waiting for a captcha to clear, and only then; the pause is reported in the failure text and in the debug window, never silent | `browser/output_wait.py` (`PauseClock`) + `tests/test_pause_clock.py`, `tests/test_output_wait_timeout_pause.py` |
...
| # | Risk | Mitigation |
|---|---|---|
| R11 | An unbounded pause hangs a job forever (user never solves) | Stop/abort honoured inside the wait (unchanged), throttled `⏸ … paused (Ns)` line, `waiting_captcha` pool row, paused worker visible in the debug window with its wait duration (D-14); per-tab Stop already exists (I-31) |
...
6. A captcha-wait **cap** setting (D-14 deliberately keeps the wait unbounded and observable).
