...
> **Measured at `6bbaf8b` with the gate's own `current_maxima()`** (rev 3): all 222 baseline entries re-measure with **0 breaches** on the seven enforced maxima, so the chain starts gate-clean. Two *soft* (unenforced) drifts exist and every line count below is written as **recorded → actual → after**: `single_job_runner.py` 902 → **939**, `undo_entries.py` 404 → **406**. Per-symbol numbers in this section are exact spans (loc / CC / nest / params), not estimates; the full 25-file table is `tdd-interfaces.md` §D and the measurement method is `evidence.md` §9.1-§9.3.
...
| `app/core/models.py` | class_loc 74, func_loc 28, cc 3 | `UrlRow:13-21` span **26 → 27** (`receiver: bool = False` appended **last**, after `tab_id`); no function span moves; `to_dict` (`:253` `asdict(u)`) and `from_dict` (`:266` `UrlRow(**u)`) need **no** edit ⇒ persistence round-trips with no migration | **100.0** | 292 → 293 |
| `app/ui/services/arena_serialize.py` | func_loc **23** | `urls_to_js` 14 → **15**; `settings_to_js` **23 → 23** (untouched) | **100.0** | 91 → 92 |
| `app/ui/services/undo_entries.py` | func_loc 17, cc 7 | `url_rows_from_js:21-32` 11 → **12**; `arena_url_rows_from_js:34-41` 7 → **8** (both `+ receiver=bool(u.get("receiver", True))` — B7's class of bug: `tab_id` was once lost the same way) | 67.85 | recorded 404 · **actual 406** → 408 |
| `app/ui/panels/app_settings.py` | func_loc **18**, methods **10**, cc 7, class_loc 118 | `save_settings:258-273` 16 → **17** (`+ apply_url_interval(self, data)`, ≤ the file max 18); the **new module function** `apply_url_interval` ≤ 10 LOC · CC ≤ 3 · 2 params (precedents: `apply_highlight_duration:82-86` 5 · CC 2, `apply_watcher_timeouts:89-100` 12 · CC 5 · nest 3); `load_arena_preset:326-344` is **18 = the file max** ⇒ nothing may be added inside it | 72.43 | 359 → **~370** |
| `app/ui/panels/layout_state.py` | func_loc 19, methods **14**, class_loc 136 | `emit_arena_state:38-47` 10 → **11** in S6 (`prog["live"] = debug_view.cadence(bridge)`) and S9 **swaps that same line** to `debug_view.live_view(bridge)` ⇒ stays 11 (no second growth, no new method) | 85.34 | 281 → **282** |
...
| `app/browser/output_wait.py` | func_loc **23**, **class_loc 4**, cc 8, nest 3, params 4 | **D-25 — no new class may live here** (`max_class_loc` **4**: `WaitSpec` span 3, `LoopState` span 4, both verified with `node_loc`). `WaitSpec:23-27` span 3 → **4** (`pause: object \| None = None`) = exactly at the maximum; `_check_timeout:151-162` 11 → **11** (first line becomes `elapsed = paused_elapsed(state.start, spec.pause)`, net-zero); `wait_for_new_output_with_spec:189-211` **untouched** (loc 23 · CC 8 · nest 3 · params 4 = all four file maxima ⇒ zero headroom). `PauseClock` itself is §4's new `app/core/pause_clock.py` | 94.95 | 211 → **~215** |
| `app/browser/cdp_arena/output.py` | func_loc **16**, params **4**, **cc 4**, **nest 1** — the tightest file in the chain | **D-26 — extraction, not branching.** `_run_wait:158-172` 14 → **15** (builds `PauseClock(cap=getattr(spec.ctrl, "pause_cap_s", 0.0))`; `PollSpec(…, pause=clock)` on the **same existing line**); `_security_gate:115-125` stays **10 / CC 4 / nest 1** because the measurement is extracted into the **new** `_settle_timed(settler, clock)` (≤ 8 LOC · CC ≤ 2 · nest ≤ 1) — an in-function `if clock is not None` would make CC 5 **and** nest 2, both breaches; `_map_wait_result:127-136` params drop **4 → 3** (`(cdp, result, spec)`) and its wording moves to the **new** `_timeout_text(spec, result)` (≤ 6 LOC · CC ≤ 3); `cdp_arena.WaitSpec:29-35` (span 7) and `mixins.wait_for_new_output:139-146` signatures **unchanged** (the cap rides `ctrl`) | 91.56 | 182 → **~188** |
| `app/services/captcha/service.py` | func_loc **27**, **cc 7**, params 4 | **D-26 — the scope gate is a split.** `handle_captcha:210` is at the file's `max_cc` **7**, so `if not in_scope: return` inside it would breach ⇒ it becomes a **4-line** scoped entry (`policy.in_scope(bridge)` lookup → delegate) and the existing body keeps CC 7 under the new name `_handle_captcha_scoped`. `_manual_wait:251` **27 → ~21** (the 7-line solved/stopped `if/else` at `:269-275` is extracted into the **new** `_wait_outcome(ctx, signal, solved, deadline)` ≤ 14 LOC; the cap rides the existing `stop` argument at `:262` — `_manual_wait`'s 4 params stay 4); `_watcher_running:242-249` → `policy`; `_resolve_captcha` 8 → 9 (D-15 reason) | 94.69 | 310 → **~322** |
| `app/persistence/config_manager.py` | class_loc 44, func_loc 13 | `DEFAULT_SESSION` module dict +1 key | 95.24 | 124 → 125 |
| `app/services/single_job_runner.py` | func_loc **26**, cc 9, nest 3 | round-1 gate edits: `check_security` 10 → 11, `_handle_security` unchanged; `wait_for_output` 19 → **20** (installs `ctrl.pause_cap_s` next to `security_settler:249`, cleared in the same `finally:257-261`); `_handle_captcha_outcome:75-85` 11 → **13** (maps `wait_timeout` → `RuntimeError(reason)`; unknown statuses already fall through, so S2's `out_of_scope` needs **no** edit here) | 83.45 | recorded 902 · **actual 939** → ~937 |
...
| `app/ui/panels/browser_tabs.py` | func_loc 20, methods 7, class_loc 76, cc 7 | round-1 D-10: `auto_scan_pass:374-387` (13 · CC 4) moves to `live/reconcile.reconcile_once` behind a 2-line delegation, `do_auto_connect_scan:389-399` (11) calls it, `report_auto_plan:390-402` (12 · **CC 7 = file max**) moves out with it; **new module functions** `live_deps(bridge) -> LiveDeps` (≤ 12 · CC ≤ 2) and `start_url_reconciler(bridge)` (≤ 8 · CC ≤ 3) — the ui→services seam (D-10) | 89.13 | 544 → **~440** |
| `app/ui/main_window.py` | func_loc 26, methods **10**, class_loc 123 | `_build_ui:48-55` 8 → **9** (`+ start_url_reconciler(self.bridge)`); `closeEvent` unchanged; **no new method** (methods 10 = the file max) | **0.0** (recorded — one of only four 0 % files: `main_window.py`, `main.py`, `cdp_arena.py`, `utils/logging.py`) ⇒ the boot line is proven by calling `start_url_reconciler(bridge)` **directly** with a fake bridge in `tests/test_live_reconcile.py`, not by constructing a `QMainWindow` | 154 → **155** |
...
|---|---:|---|---|
| **S3** `app/core/pause_clock.py` (**D-25** — `output_wait.py`'s `max_class_loc` is 4, so no new class may live there) | 60 | `PauseClock.note` ≤ 8 / CC ≤ 3 (charges `min(seconds, cap − total)`); `expired` ≤ 3; `remaining` ≤ 3; `paused_elapsed` ≤ 5; `describe` ≤ 6 — ≤ 6 methods, a pure value object (no Qt, no bridge) | `tests/test_pause_clock.py::test_second_settle_in_the_same_wait_adds_nothing` |
...
| **S6** (`interval_ms`, `clamp_interval_ms`, `cadence`) + **S9** (`live_view`, `next_queued`, `receiver_counts`) `app/services/live/debug_view.py` | 45 → **90** | `live_view` ≤ 20 LOC / CC ≤ 6 / **1 param**; `cadence` ≤ 8 / CC 2; `interval_ms` ≤ 8 / CC 3; `next_queued` ≤ 6 / CC 2; `receiver_counts` ≤ 6 / CC 2; `clamp_interval_ms` ≤ 5 / CC 2 — S9 **adds functions**, it never grows S6's (D-24a's Python analogue) | `tests/test_live_debug_view.py::test_next_queued_is_first_eligible` |
...
| **S6** `app/services/live/reconcile.py` (`LiveDeps`, `Report`, `start_reconciler`, `reconcile_loop`, `reconcile_once`, `last_pass_at`; the `mark_receivers` call lands in **S7**) | 215 | `reconcile_once` ≤ 28 / CC ≤ 9 / **params ≤ 3** (a spec object, not 6 args); `reconcile_loop` ≤ 16 / CC ≤ 5; `start_reconciler` ≤ 12 / CC ≤ 4 (idempotent, mirrors `watcher_solver.solver_start:128-137`); `LiveDeps`/`Report` spans ≤ 8/≤ 10; `last_pass_at` ≤ 4 | `tests/test_live_reconcile.py::test_interval_read_every_pass` |
| **S6** `app/services/live/url_policy.py` (`RemovalSpec`, `Removal`, `removable_rows`, `advance_misses`, `dedupe_rows`, `add_rows`, `remember`, `restore_enabled`, `removal_lines`; `mark_receivers`/`receiver_reason` land in **S7**) | 185 | `removable_rows` ≤ 20 / CC ≤ 8 (predicate table, **no `elif`** — locked by a source test); `advance_misses` ≤ 14 / CC ≤ 5; `dedupe_rows`/`add_rows` ≤ 12 each (**moved**, not rewritten, from `url_queue._dedupe_state_rows`/`_add_missing_rows`); `mark_receivers` ≤ 10 / CC ≤ 4; `receiver_reason` ≤ 8 / CC ≤ 4 | `tests/test_url_policy.py::test_mark_receivers_flags_unchecked_and_offline` |
...
| **18.2 file 150-300 LOC** | `layout_service.py` **300** (at ceiling), `single_job_runner.py` **939** (baseline records 902), `cooldown_service.py` 787, `browser_tabs.py` 544, `batch_orchestrator.py` 492, `undo_entries.py` **406** (records 404), `run_state.py` 417 (legacy, reason comments) | `layout_service.py` → **~272** ✓; `browser_tabs.py` → ~440; `batch_orchestrator.py` → ~445; new files 40-215 ✓; `single_job_runner.py`/`cooldown_service.py` unchanged (legacy debt, not worsened — RULE 16 §16.5) | ✓ direction improved |
| **18.3 module 5-15 files** (counts move in S2 `captcha` +1, S3 `core` +1, S4/S5/S6 `live` 7 files, S8 `core` +1) | `app/core` **13**, `app/services` **12** + 6 packages, `app/services/captcha` **6**, `app/browser` **23** (already over), `app/ui/panels` **16** (already over), `app/ui/services` **9** | `app/core` → **15** ✓ (`pause_clock.py` S3 + `window_catalog.py` S8); `app/services/live` → **7** ✓; `app/services/captcha` → **7** ✓; `app/browser` → **23 unchanged** (D-25 puts `pause_clock.py` in `core`, which is also why `browser` does not become 24); `app/ui/panels` → **16 unchanged** (no new panel file — `live_deps` and `start_url_reconciler` are module functions in existing panels) | ✓ two pre-existing over-ideal modules are **not worsened** (RULE 16 §16.5) |
| **18.2b sub-150-LOC files** | `bus.py` 90, `captcha/policy.py` 35 (round 1) | `window_catalog.py` 45, `debug_view.py` 90, `captcha/policy.py` 55, `live-debug/actions.js` 60, `store.js` 80, `url-list/interval.js` 60, `core/pause_clock.py` **60** (D-25 — it cannot live in `output_wait.py`, whose `max_class_loc` is 4, and as its own file it is a single-decision value object) | deliberate: each is **one decision** (a table, a clock, a predicate trio, one window's actions/cache) — splitting further would scatter twins that always change together, which is what the 150-300 band protects against |
...
| Test | Covers |
|---|---|
| **S1** `tests/test_page_pool_join.py` | **L-1**: `connect_page_pool` schedules via `run_state.schedule_coro` on a host **without** `_schedule_coro` (fails on `6bbaf8b`); the coroutine reaches the pool. **D-27/L-6**: the same commit replaces the five `_schedule_coro = queued.append` doubles in `tests/test_panel_browser_tabs.py:82,156,168,180,211` with a `monkeypatch` **spy** on the real scheduler, so those five expectations test the production path |
...
| **S3** `tests/test_pause_clock.py` | `note` accumulates, ignores ≤ 0, never raises; **charges at most `cap − total`** so a second settle in the same wait adds nothing (R22); `expired()`; `remaining()`; `paused_elapsed` subtracts; `describe` wording — all against the **new** `app/core/pause_clock.py` (D-25), plus one test that `output_wait.WaitSpec.pause` is the carrier and `LoopState` was not touched |
| **S3** `tests/test_cdp_output_settle.py` (or folded into `test_output_wait_timeout_pause.py`) | **D-26 extractions**: `_settle_timed` charges only when a settler exists and only for the settle's own duration; `_timeout_text` produces `Timeout after …ms +Ns captcha wait (cap …s)` and `paused_s`; `_map_wait_result` takes **3** params; `_security_gate`'s span/CC/nest are unchanged (source-level lock, `tdd-interfaces.md` §S3) |
...
| characterization goldens (`tests/goldens/*`) | expected **byte-identical**; regenerate only with a reviewed diff (`UPDATE_GOLDENS=1`) | no golden contains `"Timeout"` (S3's wording is safe); `check_golden:181-192` excludes **logs** but `collect_trace:158-172` includes `run_state` ⇒ S5's single-writer move must swap `RUNNERS:208` to `run_live` + arm `_stop_after` (`arm_hooks:211-231`) in the same commit, or every golden's `run_state` field drifts |
| `tests/characterization/harness.py:105-121` | `build_bridge(..., watcher_on: bool = False)` — a **new keyword with a default**, so all 10 existing callers are untouched | S2's scope predicate must be armable from a test; the harness builds a **real** `Bridge`/`ConfigManager` and never arms the Watcher today, so without this the goldens would silently change meaning |
| `tests/test_captcha_boundaries.py` (call sites) | pass `{"watcher_enabled": True}` where the case is about captcha handling | keeps those cases testing what they test after S2 makes OFF = zero activity (D-23) |
| `tests/test_captcha_pause_resume.py:124-132` | `build_bridge(..., watcher_on=True)`; pinned markers unchanged | idem; the file's marker list (`:124-132`) stays as-is |
| `tests/test_panel_browser_tabs.py:82,156,168,180,211` | **L-6 de-mask** (S1): spy on the real scheduler instead of replacing it; the idle-only prune assertions move to `removable_rows` cases in S6 | a test may not double the seam under test (D-27); the passive prune is replaced by reasoned removal (D-4) |
| `tests/test_run_control*.py` / `tests/test_panel_queue*.py` reset cases | expectations change from "reset parks the queue" to "reset ⇒ `pending` + `selected=True` + `commit_queue` + wake" | **D-6R** (owner reversal) — semantics genuinely change in S4 |
| `tests/test_batch_orchestrator.py` lifecycle cases | the run-lifecycle assertions move to `tests/test_live_supervisor.py`; the batch-mechanics cases stay | S5 moves `run_batch`/`_cancel_batch`/`_crash_batch` to the supervisor and deletes `_abort_no_tab` |
| `tests/js/test_cdp_store*.mjs` (if any asserts the 15 s timer) | drop that assertion, add "no `setInterval(…autoConnectScan…)` and exactly one `setInterval` left" | S6 deletes `cdp.js:33`; the 500 ms `ensurePrimary` tick stays (§12 item 5) |
| `package.json` → `scripts["test:js"]` (again, at **S10**) | 28 → **30** *only if* `test_captcha_saved_page.mjs` and `test_title_fit.mjs` are green | **L-7/D-27b**: two JS tests exist but never run; adopting them silently while red would be gaming |
