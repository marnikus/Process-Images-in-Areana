...
> **Re-run on the integrated tree (cycle R2, 2026-09-19).** The tooling below was
> ported to the cycle branch (`tools/mutmut_scopes.txt` re-pointed at the C2/C3
> packages `cdp/`, `cdp_arena/`, `watcher_pkg/`) and re-run there with
> **mutmut 3.8.0**. `d5-results/branch-d/` keeps this branch's original
> summaries; the files at the top level are the integrated-tree runs.
>
> | scope | Area D branch | integrated tree (re-run) |
> |---|---|---|
> | recording | 62.0 % → 74.9 % (1,811 mutants) | **82.0 %** (1,573/1,919) |
> | captcha | 59.0 % (2,774) | **62.6 %** (1,743/2,783) |
> | services-watch | 55.0 % (333) | **63.3 %** (233/368) |
> | cdp / core / services-cooldown / services-run / services-misc | 51.9 / 44.1 / 62.8 / 48.3 / 75.1 % | not re-run — queued exactly as in §Next below (one scope ≈ 8 min for the pure ones, the browser-heavy ones are much slower on this 2-core box) |
>
> Every re-run scope improved: the integrated tree carries A's converged
> pipeline, B's and D's tests and the same code, so more mutants die.

**Status (on D's own branch):** tooling + measurement + first targeted-test round **complete**;
...
- Tooling version used for every recorded run: **mutmut 3.8.0** (not pinned in
  `requirements.txt`; this lane is manual and never part of the push budget).
- The scope map must point at the *real* module paths: on the integrated branch
  the CDP monoliths are shims over `cdp/` + `cdp_arena/` packages and the watcher
  loop lives in `watcher_pkg/`, so a stale path would silently mutate nothing.
