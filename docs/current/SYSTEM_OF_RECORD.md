...
| 6 | **Run controls** | Start / Pause / Resume / Stop after current / Cancel / Retry failed. State machine 00-22 (see §3). Stop honoured inside inner waits (RULE 7). |
...
| 9 | **Action Blocks stack** | 14 block types from Old App, adapted: `CHECK_SECURITY`, `OBSERVE_BASELINE`, `HIGHLIGHT_ATTACH`, `ATTACH_IMAGE`, `VERIFY_ATTACHMENT`, `HIGHLIGHT_PROMPT`, `INSERT_PROMPT`, `VERIFY_PROMPT`, `HIGHLIGHT_SUBMIT`, `SUBMIT`, `WAIT_OUTPUT`, `DOWNLOAD`, `VALIDATE`, `SAVE`. Each has `enabled`, `selector`, `color`, `timeout_ms`, `required`, `custom_name`, `pre_delay_ms`, `highlight_duration_ms`. Plain instance attributes (RULE 3). Order drag-drop, enable toggle, undoable. |
...
| 14 | **Output detection** | Baseline capture before submit: `OBSERVE_BASELINE` stores `srcs` of existing `div.no-scrollbar img[src*=".r2.cloudflarestorage.com/"]`. After submit, `WAIT_OUTPUT` polls for new `img` not in baseline, waits for `complete && naturalWidth>0`, prefers largest `naturalWidth`. GREEN rect on new output. |
| 15 | **Download + validate** | `DOWNLOAD` fetches highest-quality src via `httpx`, with timeout. `VALIDATE` checks not HTML, valid image via PIL `Image.open()`, dimensions >0. If fails, job failed, no save. |
...
|---|---|---|
| `config/arena.json` | `prompt`, `action_blocks` (14 types with params), `settings` (supported_types, ignore_suffix, highlight_duration, confirm_pause, max_attempts, download_timeout, cdp_host, cdp_port, user_data_dir) | Arena preset store — save/load/import/export with preview |
...
| `action_blocks` | Action Blocks | Stack of 14 blocks, drag-drop order, enable toggle, params (selector/color/timeout/required/custom_name/pre_delay/highlight_duration), undoable | `arena.json` action_blocks |
