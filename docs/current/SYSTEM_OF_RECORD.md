...
| 19 | **Modern UI** | Dark-mode variables.css reused. Sash-grid draggable windows via win-grip drag_indicator, splittable/mergeable/resizable sashes, dock minimized, windows menu, Grid view menu layouts default/A/B/C + Reset. 15 windows: `url_list`, `folder`, `queue`, `prompt`, `run`, `progress`, `watcher`, `log`, `settings`, `captcha`, `browser`, `action_blocks`, `block_config`, `arena_presets`, `recordings`. The `recordings` window ("Recordings — Captcha Sessions") is the research surface for captcha session recordings: session list with independent actor/result labels, evidence completeness, event timelines, deterministic first-divergence analysis, and bounded DOM diffs — the comparison workflow that answers "why does the bot fail where a human passes". Recording lifecycle: starts the moment a captcha is detected (after the gate is confirmed ON, before any injection), stops with the solve outcome (solved / manual / page_error / token_stale / auto_failed / stopped); every transition is a NAMED milestone edge (detected → task_created → token_ready → pre_injection/field_delivery → continue_clicked → dialog_cleared → acceptance_candidate | page_error | token_stale | not_accepted | auto_failed → session_end → job_result), and acceptance is two-stage: `acceptance` records the observed solve edge (a candidate, never a claim of server acceptance) while `verified`/`job` record the later image-job truth joined through CAPTCHA_JOB. Recordings are redacted on the page before crossing CDP (recaptcha token fields + secret URL params), stored under git-ignored `config/captcha_recordings/<session>/` (schema-v2 manifest + events + gzip DOM checkpoints), and never disturb the captcha flow (fail-open). Existing persisted layouts self-migrate to include it. Design: `docs/archive/2026-09-18-captcha-session-recording/design.md`. The `captcha` window ("Captcha — 2Captcha Control") is the 2Captcha config surface: enable toggle, API key (masked display `abcd****7890` only, raw key never echoed back, field cleared after save), solve timeout (minutes, 30–600 s), live balance ($, `getBalance`), statistics (detected / auto-solved / auto-failed / manual / auto success rate / balance) with a Save + ↻ Stats title-bar actions. Existing persisted layouts self-migrate to include it (missing leaf appended, `SashCore.migrate`); layouts that still carry the dropped parallel id `captcha_records` keep that window's slot under `recordings` instead of being rejected (`LEGACY_WINDOW_IDS` on both the Python and JS side, payload version 5). Sash visibility is ONE rule, ONE writer (`_syncSashes()`, called from `_applyStates()` and the MutationObserver): a sash is hidden iff its previous (left/top) sibling is hidden — closed/minimized window or an emptied split. A divider drag resizes only its two adjacent VISIBLE rows (the sash's left row + the next visible row); hidden rows keep their stored sizes and no other row moves (commit math: visible px → `100 − Σhidden` budget). Fixes + root causes: `docs/archive/2026-09-17-watcher-grid-bugfixes/`. Title-bar invariant: ─/✕ controls are ALWAYS fully visible at the right edge in any window width — title text truncates (`span.win-name`, ellipsis), secondary title items (Save/Clear/Reparse buttons, count badges) are dropped right-to-left by the single-writer `_fitTitleBars()` (one of them hidden = `fit-hidden` class) while the row overflows, and the fixed core (grip+icon+controls+gaps+padding) is < the 96px minimum window; the fitter runs on every state change, mid-drag, and app resize. Fixes + root causes: `docs/archive/2026-09-17-titlebar-controls-visibility/`. |
| 20 | **Persistence** | JSON only, no DB. `config/arena.json` (prompt + blocks + settings), `config/urls.json` (url list + status), `config/session.json` (grid_layout + window_states + preset_store + undo_history + queue + folder + selected + cooldown keys), `config/undo.json` (full undo stack), `config/cooldowns.json` (wall-clock timers + per-URL job counters, runtime-only, git-ignored), `config/2captcha.json` (2Captcha settings: enabled + api_key + solve_timeout_sec — git-ignored, 0600 best-effort, NEVER in arena.json/session.json so presets never carry the key), `config/captcha_stats.json` (captcha counters + per-site + last balance/error — git-ignored), `config/captcha_recordings/<session-id>/` (canonical schema-v2 captcha evidence — git-ignored; single writer, one recorder per tab). Atomic writes, validated on load, never brick on corrupt JSON (RULE 13). |
| 21 | **Job cycle & cooldown** | After each job the tab auto-clicks New Chat (`a[href="/image/direct"]`, visual runner RED→ORANGE), waits for full load (readyState + page ready + empty composer), then cools down per-tab: user-set minimum (default 5 min, `cooldown_min_seconds`) + stacked captcha penalty (default +15 min each, `cooldown_captcha_penalty_seconds`, this tab only). Tab is `steady` (ready) only after its countdown ends; pool UI shows live MM:SS + reset/edit per row; URL List win likewise (correction 2026-09-16): pause/penalty bar in win, live countdown + reset/edit on each URL row matched to its pool tab, one shared image queue; cancel skips cooldown. Each URL row is bound to its tab by auto-connect alone (`UrlRow.tab_id`, set on scan add/claim or the run-start rescue claim of a checked unlinked row, I-33) so the row always shows its own tab's timer even when same-site twins tie on URL match; runs record the tab's own row but **never re-bind rows** (the legacy round-robin re-link stole rows and spawned phantom rows — `docs/archive/2026-09-18-url-row-tab-ownership/design.md`). The row checkbox is the "use for job" gate: single mode, tab-resolve, and parallel dispatch only pick tabs owned by checked rows (`enabled_tab_ids` + `resolve_primary_tab(allowed)` + `_acquire_free_in`); Start refuses with an actionable message when nothing checked owns a live tab. The finish log prints the breakdown (`total = base + captcha xN`). Captcha is also waited + recorded at submit/download boundaries. Load balancing: each finished job increments the tab's persistent counter and the next job goes to the free tab with the lowest count (ties keep pool order); Jobs column shows the counter. Design: `docs/archive/2026-09-16-job-cooldown/design.md` + `correction-url-list.md`. Silent-miss diagnosis: `fix-persist-silent-miss.md`. |
| 22 | **Captcha session recording** | Every visible captcha encounter opens one canonical schema-v2 recording at detection and closes it with the solve outcome. The bounded, thread-safe CDP collector stores redacted network metadata, lifecycle evidence, semantic solver events, and gzip DOM checkpoints under git-ignored `config/captcha_recordings/<session-id>/`; raw tokens, headers, form values, query secrets, and response bodies are excluded. The canonical `recordings` window shows every locally retained session, provides independent user-owned actor/result labels, evidence-completeness gates, deterministic first-divergence comparison and bounded DOM diffs, and removes one session immediately or all sessions after the sole confirmation prompt; every deletion is staged as one persistent undo operation, and active sessions cannot be removed. The older `app/services/recording` in-page implementation was deleted on 2026-09-18: one encounter has exactly one recorder, started by the captcha service and resolved by the CAPTCHA_JOB join. Recording is fail-open (RULE 9). Design and diagnostic: `docs/archive/2026-09-18-captcha-session-recording/design.md` and `docs/archive/2026-09-18-captcha-solve-comparison-diagnostic/verification-and-problem-diagnostic.md`. |
...
5. **Run batch**: `Bridge._do_run_batch()` loops over selected images:
   ...
   - Execute action blocks stack in order:
     ...
     - `HIGHLIGHT_ATTACH` + `ATTACH_IMAGE` — discover the visible prompt's form, mark only its hidden image input, then `DOM.setFileInputFiles` with an absolute path; reject protocol errors and require expected input filename or a new active-form preview before prompt/submit
...
| I-34 | Captcha recordings are bounded, local, orthogonal, and single-owner — no headers/form values/raw tokens; recorder failure never changes solve/manual outcome; actor and result labels are independent user-owned ground truth, never inferred; one recorder per encounter (legacy implementation removed) | New (2026-09-18) | `app/services/captcha_recording/*` + dedicated QWebChannel adapter (`captchaRecordings`); no recording slots on the main Bridge |
| I-35 | Reference image and prompt are one gated submission — attachment targets only the visible prompt form, explicit CDP errors fail, unrelated/old images cannot verify upload, and required attach failure stops before text insertion, Submit, or generation wait | New (2026-09-18) | `app/browser/attachment_probes.py` + `attachment_js/*` + required block runner — `docs/archive/2026-09-18-image-reference-paste-regression/design.md` |
...
| `config/captcha_recordings/<session-id>/` | schema-v2 `manifest.json`, event chunks, gzip DOM checkpoints | Canonical local redacted lifecycle evidence; git-ignored, bounded, orphan-active recovery, reserved terminal evidence |
...
* `docs/archive/2026-09-18-captcha-session-recording/design.md` — bounded redacted session recordings: in-page DOM/network probes installed at detection, encounter lifecycle to the solve outcome, local `logs/recordings/` store, `recordings` window with timeline + snapshot diff, user-owned `bot pass` / `manual pass` labels
* `docs/archive/2026-09-18-captcha-session-records-fix/design.md` — recording-system fix (D1–D9 audit + H1–H8 hypothesis map): one canonical recorder (legacy split-brain deleted), named milestone vocabulary with reserved terminal evidence, two-stage acceptance (solve-edge candidate resolved by the CAPTCHA_JOB join, `verified`/`job`), pre/post-inject and manual field-fill evidence, neutral bounded collection, and milestone-aligned comparison
...
| `recordings` | Captcha Session Records | Session list with solve-edge verdict vs job verdict, independent actor/result labels, completeness warnings, milestone alignment (named edges, expected provider/human differences marked), deterministic first-divergence and bounded DOM/network diff | `config/captcha_recordings/` |
