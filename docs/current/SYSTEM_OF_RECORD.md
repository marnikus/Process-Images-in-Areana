...
| I-32 | Queue _AI filters — toolbar Drop _AI removes generated outputs from the queue, Only _AI keeps just them (numbered `_AI_2` included); files on disk untouched, undoable, re-scan restores | New | `filter_queue_drop_ai` + `filter_queue_keep_ai` + `app/utils/queue_filter.py` |
