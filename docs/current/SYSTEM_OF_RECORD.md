...
* `docs/archive/2026-09-18-recaptcha-page-mechanics/research-design.md` — follow-up research/design: CAPTCHA is layered (Enterprise bootstrap, anchor, challenge, response field, callback, page/backend acceptance, generation output); callback + dialog gone is not whole-job success; late token/page-error state machine planned, no implementation yet; `SolveOutcome` carries the lifecycle (polls, token fp, dialog state, inject, mid-solve error)
