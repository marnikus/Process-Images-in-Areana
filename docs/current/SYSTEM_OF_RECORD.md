...
| 11 | **CDP connection** | Scans only the host:port from Settings (`strict_host`, e.g. `127.0.0.1:9223`); Diagnose still probes `localhost` to explain an empty result. User starts Chrome: `C:\Program Files\Google\Chrome\Application\chrome.exe --remote-debugging-port=9222 --user-data-dir="C:\arena-images-chrome"`. App connects via `ws://127.0.0.1:9222/json`. `CDPClient` with `asyncio.Lock` to prevent concurrent connect race (fixed). Tab matching: best match by URL, shows connection status per URL. Reuse check + debounce. |
| 12 | **Security / CAPTCHA** | `CHECK_SECURITY` block detects `div[role="dialog"][data-state="open"]` containing "Security Verification" or `iframe[title="reCAPTCHA"]`. If visible, pause run, emit `USER_ACTION_REQUIRED`, show page to user, wait for disappearance. Never bypass (RULE 20). |
...
| 22 | **Auto-connect & URL parsing** | No manual URL connection. On app start, on a timer (`autoconnect_interval_ms`, default 5 s) and on Chrome new-tab events (`Target.targetCreated/Destroyed/InfoChanged` via the browser endpoint), the app reads `/json/list` on the configured `cdp_host:cdp_port` **only**, keeps every page whose URL contains the stored pattern (`autoconnect_url_pattern`, default `arena.ai`; comma/semicolon/space/`|` separated list, `*`/`?` wildcards, title as secondary haystack), and links it: always into the PagePool with a dedicated client, plus the primary session when that session owns no tab (`autoconnect_primary`). Pages are identified by **unique CDP page id** — the same URL open twice is two pages; repeated ids are collapsed and counted. `devtools://`, `chrome://`, `chrome-extension://`, `about:`, `edge://` never link. Each pass reconciles: pooled pages are adopted (never re-dialled), new matches are linked, auto-added pages that closed or stopped matching are removed unless BUSY/WAITING; manually added pages are never removed. Empty (`0 matched`) is reported apart from broken (fetch error) per RULE 4. All knobs storable in `config/session.json` + arena preset JSON (`autoconnect` block). Design: `docs/archive/2026-09-16-auto-connect-url-parsing/design.md`. |
| 21 | **Job cycle & cooldown** | After each job the tab auto-clicks New Chat (`a[href="/image/direct"]`, visual runner RED→ORANGE), waits for full load (readyState + page ready + empty composer), then cools down per-tab: user-set minimum (default 5 min, `cooldown_min_seconds`) + stacked captcha penalty (default +15 min each, `cooldown_captcha_penalty_seconds`, this tab only). Tab is `steady` (ready) only after its countdown ends; pool UI shows live MM:SS + reset/edit per row; URL List win likewise (correction 2026-09-16): pause/penalty bar in win, live countdown + reset/edit on each URL row matched to its pool tab, one shared image queue; cancel skips cooldown. Design: `docs/archive/2026-09-16-job-cooldown/design.md` + `correction-url-list.md`. |
...
| I-28 | Page identity is the CDP page id, never the URL — the same URL open in two tabs is two pages; repeated ids collapse to one | New | `autoconnect_match.page_id` / `select_pages` |
| I-29 | Scans stay on the configured endpoint — `fetch_tabs(host, port, strict_host=True)` never falls back to another host while linking | New | `cdp_client.candidate_hosts` |
| I-30 | A re-scan adopts, never re-dials — pooled pages keep their live websocket; only unknown page ids are connected | New | `PageLinker.link_one` |
| I-31 | Auto-connect reaps only its own pages, and never a busy one — closed/non-matching auto-linked pages leave the pool, BUSY/WAITING and manually added pages stay | New | `PageLinker._drop_if_idle` |
| I-32 | Empty is not broken — `0 matched` logs the pattern and counts; a fetch error logs the endpoint and the reason, and the next pass retries | New | RULE 4 + `autoconnect_report.summarize_scan` |
| I-33 | An auto-connect dial never blocks the event loop — the websocket handshake runs in a worker thread, so a dead tab cannot freeze the UI or the scan timer | New | `PageLinker._dial` / `_handshake` |
| I-34 | An empty answer is confirmed before it is believed — when the async client returns no tabs the endpoint is read again synchronously; a refused port raises (broken) while Chrome with zero pages returns `[]` (empty) | New | `autoconnect_pages.fetch_pool_tabs` / `confirm_empty` |
...
| `config/session.json` | `grid_layout`, `window_states`, `window_preset_store`, `undo_history`, `folder`, `images` (queue), `selected_ids`, `prompt`, `settings` overrides, `stats`, `cdp_host`, `cdp_port`, `cdp_user_data_dir`, `cdp_extra_args`, `autoconnect_enabled`, `autoconnect_url_pattern`, `autoconnect_interval_ms`, `autoconnect_max_pages`, `autoconnect_primary` | Session persistence — layout + queue + undo + connection settings |
| `config/arena_presets.json` | Arena presets: `urls`, `folder`, `prompt`, `settings`, `images`, `cdp` (host/port/user_data_dir/extra_args), `autoconnect` (enabled/url_pattern/interval_ms/max_pages/connect_primary), `action_blocks`, `cooldown` | Whole connection + run behaviour travels with the preset |
| `config/cooldowns.json` | Per-tab wall-clock cooldown timers | A restart never frees a cooling tab |
...
| **browser** | `app/browser/cdp_client.py`, `cdp_arena.py`, `dom_highlight.py`, `site_adapter.py`, `autoconnect_match.py`, `autoconnect_config.py`, `autoconnect_linker.py`, `autoconnect_service.py`, `autoconnect_report.py`, `tab_events.py` | CDP connection with lock, visual runner (RULE 1), selector map (RULE 21), page detection + auto-linking (spec 01-04) | core, services, stdlib, websockets |
...
| **ui** | `app/ui/bridge.py`, `bridge_autoconnect.py`, `autoconnect_pages.py`, `autoconnect_settings.py`, `main_window.py`, `panels/*.js`, `web/js/*.js`, `web/css/variables.css` | PyQt6 + WebChannel, sash-grid, win-grip, dark mode, rect overlay | all below via bridge |
...
| `tests/unit/test_autoconnect_match.py` | Pattern compile/normalize, substring + wildcard match, page id extraction, duplicate URL vs duplicate id, devtools filtered, max pages, empty ≠ error | Spec 02 + I-28 |
| `tests/unit/test_autoconnect_config.py` | Defaults, interval/port clamping, `config_from_getter`, session round-trip | Spec 04 + RULE 10 |
| `tests/unit/test_autoconnect_linker.py` | Link/adopt/fail outcomes, reap on close + pattern change, busy and manual pages kept, retry after refusal | I-30, I-31 |
| `tests/unit/test_autoconnect_service.py` | One pass, broken endpoint, zero match, timer re-scan, event wake, thread-safe wake, prompt stop, single loop | Spec 03 + RULE 4/7 |
| `tests/unit/test_tab_events.py` | Browser endpoint discovery, target-event filter, subscribe + forward, reconnect after drop, stop | Spec 03 |
| `tests/unit/test_bridge_autoconnect.py` | Boot run links every match, same URL twice → two pages, primary not stolen, no re-dial, cooldown restored, settings persist + clamp, preset round-trip, shutdown | Spec 01/03/04 |
| `tests/integration/test_autoconnect_endpoint.py` | Whole chain against a real HTTP `/json/list` endpoint (no Chrome): matches only the pattern, duplicate URL → two pages, devtools skipped, new tab linked on the next pass, closed tab reaped, closed port reported broken | Spec 01-04 + I-28..I-34 |
...
* `docs/archive/2026-09-16-auto-connect-url-parsing/design.md` — auto-connect & URL parsing (endpoint scan, pattern filter, page-id identity, reconcile, new-tab events)
...
| `settings` | Settings | Supported types, ignore suffix, highlight duration, confirm pause, max attempts, download timeout, CDP host/port, user-data-dir, **Auto-Connect & URL Parsing block** (enable, URL pattern field, re-scan interval ms, max pages, primary session, Save, Scan Now, last-scan status line) | `session.json` settings + `autoconnect_*` keys |
| `cdp` | Connection | Chrome debug port status, tab list, best match, connect/disconnect, user decides port and --user-data-dir; **Auto-Connect toolbar** above the grid shows badge (watching/off/port unreachable), live pattern + endpoint + interval, scanned/matched/linked counts, Scan Now and On/Off | settings cdp_host/port/user_data_dir + `autoconnect_*` |
| `help` | Help | Manual, selector map, workflow, shortcuts, about | static |

**Layout:** Sash-grid draggable via win-grip `drag_indicator`, splittable/mergeable/resizable sashes, dock minimized, windows menu, Grid view menu layouts default/A/B/C + Reset, Window presets save/load/import/export with preview. Persistence `grid_layout+window_states+window_preset_store` in `config/session.json`. From Old App, tested.

**Dark mode:** `app/ui/web/css/variables.css` reused from Old App — same variables, same dark theme.

---

*Last updated: 2026-09-16 — auto-connect & URL parsing (row 22, I-28..I-34). This file is the current truth — if not true today, it does not belong here (RULE 17). Archive old truth to `docs/archive/<date>-<topic>/`.*
