...
| 22 | **Captcha session recording** | Every visible captcha encounter opens a bounded local recording at detection and closes it with the solve outcome (`solved` / `manual` / `page_error` / `token_stale` / `auto_failed` / `stopped`). In-page probes (installed before any injection) capture timestamped DOM mutation summaries, fetch/XHR request lines and sanitized HTML snapshots; snapshots are redacted in the page before bytes cross CDP (recaptcha token fields + secret URL params) and land under git-ignored `logs/recordings/<session-id>/` (`session.json` header + `events.jsonl` + `NNNN.html`). The `recordings` window lists sessions, shows the event timeline + snapshots, renders a bounded unified diff between any two snapshots, and stores the user's ground-truth label (`bot pass` / `manual pass` / cleared) — labels are never inferred. Recording is fail-open (RULE 9): no recorder error can change the solve outcome. Design: `docs/archive/2026-09-18-captcha-session-recording/design.md`. |
...
| I-34 | Captcha recordings are bounded, local, and orthogonal — no headers/form values/raw tokens; recorder failure never changes solve/manual outcome; actor label is user-set ground truth, never inferred | New (2026-09-18) | `app/services/recording/*` + `app/browser/recording_js/*` (`_recording_service` bridge slots) — `docs/archive/2026-09-18-captcha-session-recording/design.md` |
...
| `logs/recordings/<session-id>/` | `session.json` header + `events.jsonl` + `NNNN.html` redacted snapshots (+ `settings.json` enable flag) | Local redacted captcha lifecycle evidence; git-ignored, redacted in-page before storage, fail-open |
...
| `recordings` | Recordings — Captcha Sessions | Session list, editable labels, event timeline, snapshot viewer and unified diff between two sessions | `logs/recordings/` |
