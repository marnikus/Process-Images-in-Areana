...
5. **Run batch**: `Bridge._do_run_batch()` loops over selected images:
   ...
   - Execute action blocks stack in order:
     ...
     - `HIGHLIGHT_ATTACH` + `ATTACH_IMAGE` — paste a bounded image `File` into the visible prompt composer, then fall back to its marked hidden input with `DOM.setFileInputFiles`; reject protocol errors and require expected input filename or a new active-form preview before React-persistent prompt insertion and submit
...
| I-34 | Captcha recordings are bounded, local, orthogonal, and single-owner — no headers/form values/raw tokens; recorder failure never changes solve/manual outcome; actor and result labels are independent user-owned ground truth, never inferred; legacy recorder is fallback-only when schema v2 is unavailable | New (2026-09-18) | `app/services/captcha_recording/*` + dedicated QWebChannel adapter; `app/services/recording/*` compatibility fallback |
| I-35 | Reference image and prompt are one gated submission — attachment uses active-composer paste then a marked-input fallback, explicit CDP errors fail, unrelated/old images cannot verify upload, prompt text must persist through the controlled-input render, and either failure stops before Submit or generation wait | New (2026-09-18) | `app/browser/attachment_probes.py` + `attachment_js/*` + required block runner — `docs/archive/2026-09-18-image-reference-paste-regression/design.md` |
...
| `config/captcha_recordings/<session-id>/` | schema-v2 `manifest.json`, event chunks, gzip DOM checkpoints | Canonical local redacted lifecycle evidence; git-ignored, bounded, orphan-active recovery; legacy fallback may use `logs/recordings/` |
...
| `recordings` | Captcha Session Records | Session list, independent actor/result labels, completeness warnings, deterministic A/B divergence and bounded DOM evidence comparison | `config/captcha_recordings/` |
