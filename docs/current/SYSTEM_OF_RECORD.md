...
* `docs/archive/2026-09-18-captcha-reporting/design.md` — round 10: structured reporting — one `CAPTCHA_SOLVE` JSON line per encounter (detection + task + token + edge data, joined by eid) + one `CAPTCHA_JOB` line at image-job end (outcome + page error); `SolveOutcome` carries the lifecycle (polls, token fp, dialog state, inject, mid-solve error)
