...
|---|---|---|
| 1 | **URL list** | List of URLs user pastes. Each validated: syntax, reachability via CDP, auth (not sign-in page), readiness (textarea + send button visible, no security dialog). Validation statuses remain; batch page states are `checking / steady / busy / waiting_captcha / not_ready / unavailable / unchecked`. `steady` requires two consecutive live idle observations; persisted status is never admission authority. Manual include/exclude. Persisted in `config/urls.json` + `config/session.json`. |
...
| 6 | **Run controls** | Start / Pause / Resume / Stop after current / Cancel / Retry failed. State machine 00-22 (see §3). Free-page workers run concurrently on dedicated CDP connections. Pause prevents new assignments; Stop drains current page jobs without new claims; Cancel joins all workers before returning idle. Restart is rejected while running, paused, or stopping. Stop honoured inside inner waits (RULE 7). Submit button handling improved 2026-09-16: primary `button[aria-label="Send message"]:not([disabled])` + fallback list comma-separated, extra 0.8s delay + wait for enabled (React), multiple click dispatch methods. |
...
| 11 | **CDP connection** | User starts Chrome: `C:\Program Files\Google\Chrome\Application\chrome.exe --remote-debugging-port=9222 --user-data-dir="C:\arena-images-chrome"`. App connects via `ws://127.0.0.1:9222/json`. `CDPClient` with `asyncio.Lock` to prevent concurrent connect race (fixed). Manual tab matching retains best-match behavior. Batch dispatch uses exact configured URLs (origin casing/fragment normalized, conversation path/query preserved), deduplicates physical target IDs, and opens one dedicated socket per worker. Missing/duplicate/failed matches are unavailable. A shared deque assigns the next image to the next confirmed-free page; a failed or returned job does not release the page until browser activity has finished. Broken observations quarantine that page; other pages continue. The single-tab watcher yields during a batch. Reuse check + debounce applies to manual connections. |
| 12 | **Security / CAPTCHA** | `CHECK_SECURITY` block detects `div[role="dialog"][data-state="open"]` containing "Security Verification" or `iframe[title="reCAPTCHA"]`. If visible, wait for manual disappearance on that page; other page workers continue. Before assignment, CAPTCHA makes the page `waiting_captcha` and receives no work. Never bypass (RULE 20). |
...
*Last updated: 2026-09-15. This file is the current truth — if not true today, it does not belong here (RULE 17). Archive old truth to `docs/archive/<date>-<topic>/`.*

Multi-page dispatch design and verification: `docs/archive/2026-09-15-free-page-dispatch/design.md` and `docs/archive/2026-09-15-free-page-dispatch/verification.md`.
