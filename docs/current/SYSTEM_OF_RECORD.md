...
* `docs/archive/2026-09-18-captcha-escalation-path/design.md` — "research the saved page: are there more captcha mechanics? solve the FULL path": arena's bundle source (saved webpage chunks) reveals the complete detection stack — Cloudflare edge, invisible v2 badge, per-request enterprise-execute token, and the visible v2 escalation dialog (triggered by the server answer `error: "recaptcha validation failed"`, dialog sitekey `6Le3_cYs…`, 60 s window; the render options' `callback` closure is the only thing that closes the dialog and retries the request with `recaptchaV2Token`). The solve now completes that canonical path: CDP attach installs an idempotent render hook capturing the closure (`window.__arenaV2Challenge`), injection resolves it with the 2Captcha token (`path:"hook"` — arena itself re-POSTs the original request), field-patch (`path:"field"`) and force-close+re-send stay as fallbacks; detect/scan logs show hook evidence + the 60 s window budget
