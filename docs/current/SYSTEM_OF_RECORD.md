...
| I-18 | Selector priority semantic > structural > class fragment — all live selectors in the GENERATED map in `site_adapter.py` (primary + fallbacks + `tier` + `evidence`), produced by `tools/generate_selectors.py` from the live probe chains and kept in lockstep by the pre-push `--check` gate | New (2026-09-19) | RULE 21 |
