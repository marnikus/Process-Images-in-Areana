...
| 12 | **Security / CAPTCHA** | CAPTCHA is Watcher-owned. When `WatcherConfig.enabled` is false, the watcher performs no CAPTCHA probe or provider call and the image pipeline performs no CAPTCHA work. When enabled, the Watcher probes every connected page independently, records per-page waiting state, and may submit only a visible supported signal through the official `2captcha-python` SDK after the user enables it and stores their own key. SDK calls run off the event loop, are deduplicated per tab, inject only through the Watcher CDP boundary, and failures/missing keys/sitekeys fall back to visible manual handling. The image job runner treats `CHECK_SECURITY` as an informational/restorable block and never detects, solves, waits for, or revives a CAPTCHA. Tokens and keys never enter logs, UI payloads, presets, or recordings. Design and implementation: `docs/archive/2026-09-19-captcha-watcher-isolation/`. |
