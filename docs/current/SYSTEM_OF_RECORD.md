...
| 22 | **Captcha session recording** | Every visible captcha encounter on a CDP-backed tab starts a bounded local recording after detection and ends on solved/manual/error/stale/stopped/interrupted resolution. It writes an atomic manifest, append-only timestamped DOM mutation and sanitized network lifecycle events, capped textual response bodies, and gzip sanitized DOM checkpoints under git-ignored `config/captcha_recordings/<session-id>/`; oldest sessions are pruned above 200 sessions or 512 MiB total. URL queries, headers, form values, scripts, credentials, and opaque tokens are not persisted. The `captcha_records` window lists method/outcome/timing/counts, lets the user explicitly label ground truth `unknown`, `bot`, `manual`, or `mixed` (actor) and `unknown`, `passed`, or `failed` (result); labels are never inferred. Each row has A/B comparison buttons, an Open-folder button, and a delete button (confirmation required). The comparison UI reads adapted events (`offset_ms` → `at_ms`, flat keys → `payload` dict) and shows event timeline + latest DOM checkpoint. Labels are never inferred. Recorder errors fail open and cannot alter captcha handling. Design: `docs/archive/2026-09-18-captcha-session-recording/design.md`. Fix plan: `docs/archive/2026-09-18-captcha-solve-comparison-diagnostic/implementation-plan.md`. |
