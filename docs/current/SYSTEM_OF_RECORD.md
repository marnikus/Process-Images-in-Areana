...
5. **Run batch**: `Bridge._do_run_batch()` loops over selected images:
   ...
   - Execute action blocks stack in order:
     ...
     - `HIGHLIGHT_ATTACH` + `ATTACH_IMAGE` — use the proven `032dfc6` path: query the first matching form/image file input, call `DOM.setFileInputFiles` with the absolute path, then require the established visible preview before inserting prompt text with the proven native textarea setter/events
...
| I-35 | Reference image and prompt are one gated submission — browser interaction is restored from known-working branch commit `032dfc6` (ordered file-input query + `DOM.setFileInputFiles`, native textarea setter + bubbling events); explicit protocol errors fail, and required attachment/prompt failure stops before Submit or generation wait | New (2026-09-18) | `CDPClient.attach_image_cdp` + `CDPArenaController.attach_image`/`insert_prompt` + required block runner — `docs/archive/2026-09-18-working-branch-composer-restoration/design.md` |
