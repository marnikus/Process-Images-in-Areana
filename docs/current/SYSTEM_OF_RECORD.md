...
| 22 | **Captcha session recording** | Every visible captcha encounter on a CDP-backed tab starts a bounded local recording after detection and ends on solved/manual/error/stale/stopped/interrupted resolution. It writes an atomic manifest, append-only timestamped DOM mutation and sanitized network lifecycle events, capped textual response bodies, and gzip sanitized DOM checkpoints under git-ignored `config/captcha_recordings/<session-id>/`; session count is under user control — the Records window lists every retained session (up to 1000 rows, exact total in the title bar) with a per-row delete (confirm dialog) — while a 512 MiB disk-safety cap still prunes the oldest as a last resort. URL queries, headers, form values, scripts, credentials, and opaque tokens are not persisted. The `captcha_records` window lists method/outcome/timing/counts, lets the user explicitly label ground truth `unknown`, `bot`, or `manual`, and loads any two sessions into bounded side-by-side event-timeline (offset_ms + event detail fields) and latest-DOM (checkpoint timestamp) panes matching the persisted schema; labels are never inferred. Recorder errors fail open and cannot alter captcha handling. Design: `docs/archive/2026-09-18-captcha-session-recording/design.md` + `docs/archive/2026-09-18-captcha-bot-failure-analysis/verification-and-problem-diagnostic.md`. |
...
| I-29 | Solver key never exposed — raw keys live only in git-ignored `config/captcha_solvers.json` (per-provider, 0600 best-effort; legacy `config/2captcha.json` migrated read-only); WebChannel/UI carry masked form only; key never in logs, payloads, presets, or error text; tasks go only to the selected provider's official endpoint; solver failures never block the job (fall back to manual wait) | New (2026-09-17), extended to two providers (2026-09-18) | `CaptchaKeyStore.mask` + `providers.py` specs + bridge status payload + RULE 20 amendments |
...
| I-32 | Captcha recordings are bounded, local, and orthogonal — no headers/form values/raw tokens; recorder failure never changes solve/manual outcome; actor label is user-set ground truth, never inferred | New (2026-09-18) | `captcha_recording` package + dedicated QWebChannel adapter |
...
| `config/captcha_solvers.json` | `provider`, `solve_timeout_sec`, `providers.{2captcha,capmonster}.{enabled,api_key}` (git-ignored, 0600 best-effort; legacy `config/2captcha.json` migrated read-only) | Solver-provider opt-in settings — kept out of arena.json/session.json so presets never carry keys (RULE 20) |
