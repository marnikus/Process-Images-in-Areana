...
| I-32 | Queue _AI filters — toolbar Drop _AI / Only _AI scan the Folder-picker path recursively (background, shared scan guard), merge into the queue, then drop generated outputs / keep just them (numbered `_AI_2` included); files untouched, undoable | New | `filter_queue_drop_ai` + `filter_queue_keep_ai` + `_merge_scanned` + `app/utils/queue_filter.py` |
