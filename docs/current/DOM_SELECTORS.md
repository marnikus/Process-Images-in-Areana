...
|---|---|---|
| Upload “+” → popup “Add files” item (2026-09-26) | `add_files_menu_item_selectors()` / `add_files_menu_item_text()` (`add_files_menu_item`: `[role="menuitem"]:has(svg.lucide-paperclip)`, then `[role="menu"]` / Radix popper / structural `:has(> svg.lucide-paperclip):has(> span > span)`; label “Add files”) | the + now opens a popup; `job_scripts.upload_item_js` tags its first visible item (label preferred, never the + itself) `data-arena-upload-item`, then a RED rect + XClick `css=[data-arena-upload-item]` opens the OS dialog; no tag ⇒ the + opened the dialog itself |
...
| Attachment preview | `attachment_preview_selectors()` | exactly one visible preview whose `alt` == the uploaded queue file's name (since 2026-09-26; stale / wrong / multiple ⇒ reset or retry) |
