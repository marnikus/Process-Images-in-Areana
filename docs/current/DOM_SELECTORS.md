...
> **Generated map (2026-09-19):** the executable registry is the `SELECTORS` block in
> `app/browser/site_adapter.py`, generated by `tools/generate_selectors.py` from the
> LIVE probe chains (cdp_arena, output_probes, new_chat, cdp_client, captcha_js,
> action-block defaults). Each entry carries its RULE 21 `tier` and `evidence`
> (which probes use it). Probe chains stay literal in the probe sources —
> `tests/js/*.mjs` execute the real strings (RULE 8) — and the pre-push check runs
> `tools/generate_selectors.py --check` so the map can never drift. This document is
> the human catalogue with history and evidence; when the site changes, edit the
> probe, run `--write`, and update the sections below in the same change (RULE 17).
...
| Field | Value |
|---|---|
| **Live chain (generated map)** | `send_button` = `button[aria-label="Send message"]` (state probe, readiness check); `send_button_click` = `button[aria-label="Send message"]:not([disabled])` → `form button[aria-label="Send message"]` → `button[aria-label="Send message"]` (click probe — the JS loop itself additionally requires visible + not disabled) |
| **History (2026-09-16)** | The original 9-fallback plan (`:not([disabled])` variants, `form:has(...)`, `button:has(svg)`, `button[type="submit"]` …) was consolidated: the state probe reads `.disabled` directly and the click probe verifies `el.disabled` in JS, so selector-level `:not([disabled])` and `:has()` fallbacks are redundant. The map below is what the probes actually try. |
...
All live selectors are centralized in `app/browser/site_adapter.py` as generated
`SelectorObject` entries (primary + ordered fallbacks, `tier`, `evidence`,
`lastVerified`), kept in lockstep with the probe chains by
`tools/generate_selectors.py` (pre-push `--check` gate).

---

*This document is the human catalogue; the executable registry is the generated
map in `app/browser/site_adapter.py`. Last verified: 2026-09-19 (generated map,
32 entries). Next verification when arena.ai DOM changes — edit the probe, run
`python tools/generate_selectors.py --write`, and update this file in same change (RULE 17).*
