...
- `archive/2026-09-18-captcha-solve-comparison-diagnostic/implementation-plan.md` — prioritized fix plan for comparison viewer (P1–P9), delete/view-all features, result labels, solver milestones, thread safety
- `archive/2026-09-18-captcha-solve-comparison-diagnostic/adaptive-solve-pipeline-design.md` — adaptive CAPTCHA pipeline: timing database, instant-manual for known-impossible sites, adaptive timeout, warm pre-started provider tasks, outcome honesty; §9 records the shipped status + why the concurrent error race was dropped (poll loop already re-checks page errors every round)
