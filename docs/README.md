...
- `archive/2026-09-18-capmonster-solver-provider/design.md` (+ `implementation.md`) — CapMonster Cloud added as a second solver provider: provider drop-down in the Captcha window (2Captcha | CapMonster Cloud), per-provider keys in `config/captcha_solvers.json` (legacy `config/2captcha.json` migrated read-only), `providers.py` spec table (task types, error codes, poll cadence, refund support), RULE 20 amendment 2026-09-18, `fix-provider-switch.md` (user-owned drop-down — a selection never snaps back on reload/balance failure)
- `archive/2026-09-18-passive-captcha-monitor/design.md` — run flow adopts the watcher's principle: per-page ~500 ms passive captcha monitors (bridge loop + dispatcher, SettleGuard-guarded single handling), solver poll hardened (tri-state probe, 2-check confirmation, rotation retries), `wait_captcha_cleared` must STAY clear (no more instant false 'manual' + wrongful penalty)
- `archive/2026-09-18-bridge-midwait-captcha-gate/fix.md` — bridge inline run loop never armed `security_settler`: a captcha appearing MID-wait produced no detection line and burned the whole generation timeout; fix = `InlineWaitGates` + `wait_with_gates` around all five inline waits (dispatcher parity), `_settle_captcha_at` returns bool for the revival stamp
