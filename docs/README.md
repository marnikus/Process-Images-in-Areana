...
- `archive/2026-09-19-area-b-dead-code-purge/design.md` — **Area B IMPLEMENTED 2026-09-19** (B1–B6): 3-detector deadness proof (all four modules at 0 executed statements without their own tests), deletion of controller/output_detector/job_runner/job_state_machine (−1,239 LOC), RULE 21 restored (site_adapter single source + `probe_selectors.py` payload generation + literal-lint test), vulture @90 clean, `json_store.py` extraction, output_probes payload dedup with node characterization; record with Before→After metrics: `archive/2026-09-19-code-quality-implementation-plan/implementation-2026-09-19.md`
