...
- `archive/2026-09-19-dead-code-quality-batch/design.md` + `implementation-2026-09-19.md` + `improvements-2026-09-19.md` + `-round3.md` + `-round4.md` — Batch B: 8-module dead-closure proof (`tools/import_graph.py`), deletion of 4 dead modules (job_runner, controller, job_state_machine, output_detector + playwright dep), RULE 21 selector generation (`tools/generate_selectors.py` → generated map in `site_adapter.py`, pre-push `--check` sync gate), unused-import cleanup, `app/persistence/json_store.py` atomic-JSON dedup; gate evidence (149→128 fails, zero new breaches); passes 2–4: dead-code removal on the B surface, `test_json_store.py`, RULE 18 function splits, §8/§7/CODE_VERIFICATION doc sync, pre-push hook install + solver cognitive fix, workflow-doc sync, and the final deferred work (RULE 21 class-fragment primaries resolved — fallback or documented `tierNote` debt; both pre-existing `.mjs` failures fixed — JS suite now 11/11 files green)
