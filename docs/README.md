...
- `archive/2026-09-19-code-quality-implementation-plan/` — **implementation plan, planning only, no prod code**: `design.md` (process + area map + priority table), `step1-understand-prioritized.md` (Step 1 split into 10 sub-steps 1.1–1.10 by severity P1 96→P9 33, each with measure command/evidence/decision/owner), `area-plans-prioritized.md` (Round 0 R0.1–R0.6 + Area B B1–B6 first quick win, Area A A1–A7 centre of gravity, Area C C1–C8 + D D1–D7 parallel, all ≤20 LOC steps RULE 19 order nesting→CC→cognitive→size), `verification-checklist.md` (RULE 16 hard gates + RULE 18 ideals recheck template, per-step/per-area/final), `implementation-2026-09-19.md` (implementation record — Area B filled 2026-09-19) — prioritized by blast radius×3 + evidence×2 + gate×2 + defect×2 + delay×1, implementation order Round0→B→A→C/D with D1 immediate
- `archive/2026-09-19-area-b-dead-code-purge/design.md` — **Area B IMPLEMENTED 2026-09-19** (B1–B6): 3-detector deadness proof, deletion of controller/output_detector/job_runner/job_state_machine (-1,239 LOC), RULE 21 restored (site_adapter single source + probe_selectors payload generation + lint test), vulture triage (@90 clean), json_store.py extraction, output_probes payload dedup with node characterization; record with Before→After metrics: `archive/2026-09-19-code-quality-implementation-plan/implementation-2026-09-19.md`
- `archive/2026-09-19-area-b-review-and-followups/` — **Area B round 2 (2026-09-19)**: second-pass audit of B1–B6 vs specs (findings F-1…F-7), follow-up plan B7–B12 (coverage completion, vulture hygiene, RULE 21 hardening, stale docs, output_probes dedup, gate honesty) and the B7–B12 implementation record
- `archive/2026-09-19-area-b-round3-review/` — **Area B round 3 (2026-09-19)**: third-pass audit of the full B implementation (B1–B12 all DELIVERED under fresh measurement; findings F-8…F-13), plan B13–B15 (gate-tool RULE 18 fit, pre-push node lane, doc alignment) and its implementation record
- `archive/2026-09-19-area-b-round4-review/` — **Area B round 4 (2026-09-19)**: fourth-pass audit (B1–B15 all DELIVERED under fresh measurement; findings F-14…F-17: ratchet floor lowerable by its own flag, pre-push changed-file lane never engaged, test brittleness, error-lane gaps), plan + implementation record B16–B18 (gate integrity, real changed-file gating, docs)
- `archive/2026-09-19-area-b-round5-review/` — **Area B round 5 (2026-09-19)**: fifth-pass audit (B1–B18 all DELIVERED; F-18: legacy hotspots could grow silently — proven live, a 36-LOC function added to bridge.py passed the gate; F-19: SOR §8 gap; F-20 handover), plan + amendment + record B19–B20 (per-symbol legacy ratchet — the R0.3/D6 negative test now holds)
- `archive/2026-09-19-worst-first-remediation/design.md` — **active roadmap (supersedes execution order of the area plan)**: honest diagnosis of the slow first phase (−21 fails/24 commits, bridge untouched), fresh worst-first research (64/128 fails in bridge.py, CC-356 `_do_run_batch`, 3,504 uncovered lines; D1 already fixed with regression test), efficiency principles (one shared test-doubles harness, 3 goldens not 12, every step lands a delta, zero meta-work), phases W0–W4 with per-step exit criteria and a KPI table targeting gate ≤5 and 80/75 coverage
