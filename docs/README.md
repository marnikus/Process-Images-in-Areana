...
- `archive/2026-09-18-captcha-session-recording/design.md` — bounded redacted session recordings that shipped: in-page DOM/network probes installed at detection, encounter lifecycle to the solve outcome, local `logs/recordings/` store, `recordings` window (list + timeline + snapshot diff), user-owned `bot pass` / `manual pass` labels
...
- `archive/2026-09-18-dead-generation-toast-revival/design.md` — the layer after the captcha: the blocked generation request dies server-side and the site toasts "Something went wrong while generating the response. Please try again." — that toast is now classified as a dead request (`match_dead_generation`), converted to a revival poll once per wait, and answered with the bounded one-resubmit policy instead of an instant job failure; solver failure paths keep the provider task id for traceability
- `archive/2026-09-18-captcha-recording-cdp-research/design.md` — research record for the parallel CDP-event / compressed-checkpoint recording approach that was NOT the one shipped (kept for its CDP event-routing, DOMSnapshot and retention findings)
