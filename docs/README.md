...
- `archive/2026-09-19-dead-code-quality-batch/design.md` + `implementation-2026-09-19.md` + `improvements-2026-09-19.md` — Batch B: 8-module dead-closure proof (`tools/import_graph.py`), deletion of 4 dead modules (job_runner, controller, job_state_machine, output_detector + playwright dep), RULE 21 selector generation (`tools/generate_selectors.py` → generated map in `site_adapter.py`, pre-push `--check` sync gate), unused-import cleanup, `app/persistence/json_store.py` atomic-JSON dedup; gate evidence (149→129 fails, zero new breaches); second-pass review findings + improvements (dead-code removal on the B surface, `test_json_store.py`, RULE 18 function splits, §8 doc fixes, pre-push hook install)
